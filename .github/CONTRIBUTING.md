...
- English/British spelling on var/proc names \* Color/Colour - both are fine, but keep in mind that BYOND uses `color` as a base variable
- Spaces after control statements \* `if()` and `if ()` - nobody cares!
...
- Operators that should be separated by spaces
  _ Boolean and logic operators like &&, || <, >, ==, etc (but not !)
  _ Bitwise AND &
  _ Argument separator operators like , (and ; when used in a forloop)
  _ Assignment operators like = or += or the like
- Operators that should not be separated by spaces
  _ Bitwise OR |
  _ Access operators like . and :
  _ Parentheses ()
  _ logical not !
...
- Bitwise AND - '&' \* Should be written as `bitfield & bitflag` NEVER `bitflag & bitfield`, both are valid, but the latter is confusing and nonstandard.
