		...
		switch(I.action)
			if(ACTION_EQUIP) // Put held thing in belt or take out most recent thing from belt
				...
				if(thing) // put thing in belt
					if(!equipped_belt.SEND_SIGNAL(src, COMSIG_TRY_STORAGE_INSERT, thing, user.mob))
						to_chat(user, "<span class='notice'>You can't fit anything in.</span>")
					return
			...
			if(ACTION_RESIST) // Put held thing in backpack or take out most recent thing from backpack
				...
				if(thing) // put thing in backpack
					if(!equipped_backpack.SEND_SIGNAL(src, COMSIG_TRY_STORAGE_INSERT, thing, user.mob))
						to_chat(user, "<span class='notice'>You can't fit anything in.</span>")
					return
