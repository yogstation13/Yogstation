...
handle_fuel - called in spec_life(), handles the alert indicators, and fuel loss.
*/
...
/datum/species/preternis
	...
	inherent_traits = list(TRAIT_NOHUNGER, TRAIT_MEDICALIGNORE) //Medical Ignore doesn't prevent basic treatment,only things that cannot help preternis,such as cryo and medbots
	...
	exotic_blood = /datum/reagent/oil //They're mostly machine
	yogs_virus_infect_chance = 20
	var/charge = PRETERNIS_LEVEL_FULL
	var/fuel = PRETERNIS_FUELLEVEL_FULL
...
/mob/living/carbon/human/species_types/preternis/preternis/gib_animation()
...
/datum/species/preternis/handle_chemicals(datum/reagent/chem, mob/living/carbon/human/H)
	...
	if(H.reagents.has_reagent(/datum/reagent/fuel))
		H.adjustToxLoss(-1.5*REAGENTS_EFFECT_MULTIPLIER,FALSE,FALSE, BODYPART_ANY)
		fuel = clamp(fuel + 1 * REAGENTS_METABOLISM,PRETERNIS_LEVEL_NONE,PRETERNIS_LEVEL_FULL)
...
/datum/species/preternis/proc/handle_fuel(mob/living/carbon/human/H)
	fuel = clamp(fuel - fuel_drain,PRETERNIS_FUELLEVEL_NONE,PRETERNIS_FUELLEVEL_FULL)
	speedmod = 0
	if(fuel == PRETERNIS_FUELLEVEL_NONE)
		H.throw_alert("preternis_fuel", /obj/screen/alert/preternis_fuelnone, 4)
		speedmod = 1 //System is running only on electricity, movement is heavily impeded.
	else if(fuel < PRETERNIS_FUELLEVEL_STARVING)
		H.throw_alert("preternis_fuel", /obj/screen/alert/preternis_fuel, 3)
		speedmod = 0.33
	else if(fuel < PRETERNIS_FUELLEVEL_HUNGRY)
		H.throw_alert("preternis_fuel", /obj/screen/alert/preternis_fuel, 2)
	else if(fuel < PRETERNIS_FUELLEVEL_FED)
		H.throw_alert("preternis_fuel", /obj/screen/alert/preternis_fuel, 1)
	else
		H.clear_alert("preternis_fuel")
		speedmod = 0
