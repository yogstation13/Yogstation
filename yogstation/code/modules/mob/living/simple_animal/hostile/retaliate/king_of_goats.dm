...
/mob/living/simple_animal/hostile/retaliate/goat/king/phase2/Initialize()
	. = ..()
	//boss_theme = GLOB.sound_player.PlayLoopingSound(src, sound_id, 'yogstation/sound/ambience/Visager-Battle.ogg', volume = 10, range = 7, falloff = 4, prefer_mute = TRUE)
	update_icon()
...
/mob/living/simple_animal/hostile/retaliate/goat/king/phase2/Retaliate()
	...
	if(spellscast < 5)
		if(prob(5) && move_to_delay != 1) //speed buff
			...
			new /mob/living/simple_animal/hostile/retaliate/goat/guard/master(get_step(src,pick(GLOB.cardinals)))
			new /mob/living/simple_animal/hostile/retaliate/goat/guard(get_step(src,pick(GLOB.cardinals)))
			new /mob/living/simple_animal/hostile/retaliate/goat/guard(get_step(src,pick(GLOB.cardinals)))
...
/mob/living/simple_animal/hostile/retaliate/goat/king/phase2/proc/phase3_transition()
	...
	//boss_theme = GLOB.sound_player.PlayLoopingSound(src, sound_id, 'yogstation/sound/ambience/Visager-Miniboss_Fight.ogg', volume = 10, range = 8, falloff = 4, prefer_mute = TRUE)
	stun_chance = 10
	update_icon()
