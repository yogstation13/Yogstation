/mob/living/silicon/robot/proc/PickBorgSkin(forced = FALSE) //We'll do our own AI version inside its pre existent skin selector methinks
	icon = initial(icon) //Redundancy in case they repick a skin after modelchange
	...
	if(forced || is_donator(client))//First off, are we even meant to have this verb? or is an admin bruteforcing it onto a non donator for some reason?
		if(model.name == "Default")
			to_chat(src, "Please choose a model first! (Standard works too)")
			return FALSE
		if(ROLE_SYNDICATE in faction)
			to_chat(src, span_warning("You cannot reskin as a syndicate cyborg!"))
			return FALSE
		var/datum/borg_skin/skins = list("default" = icon(icon = src.icon, icon_state = model.cyborg_base_icon))
		for(var/T in GLOB.DonorBorgHolder.skins)
			if(istype(T, /datum/borg_skin))
				...
				if(S.owner == client.ckey || !S.owner) //We own this skin.
					if(!S.model_locked || S.model_locked == model.name)
...
//I'm overriding this so that donors will be able to pick their borg skin after choosing a model.
/obj/item/robot_model/do_transform_delay()
	. = ..()
...
/mob/living/silicon/robot/examine(mob/user)
	. = ..()
	. += "It seems to have the <b>[model.name] model</b> loaded."
