...
/datum/admins/proc/makeMentor(ckey)
	...
	if(SSdbcore.Connect())
		...
		var/datum/DBQuery/query_add_admin_log = SSdbcore.NewQuery("INSERT INTO `[format_table_name("admin_log")]` (`id` ,`datetime` ,`adminckey` ,`adminip` ,`log` ) VALUES (NULL , NOW( ) , ':usrckey', ':address', 'Added new mentor :ckey');", list("usrckey" = usr.ckey, "address" = usr.client.address, "ckey" = ckey))
