		...
		if("set")
			switch(params["name"])
				if("Damage")
					...
					if((points + (saved_stats.damage > TRUE ? saved_stats.damage - TRUE : 0)) >= lvl - TRUE || lvl == 1)
						saved_stats.damage = lvl
					. = TRUE
				if("Defense")
					var/lvl = clamp(text2num(params["level"]), 1, 5)
					if((points + (saved_stats.defense > TRUE ? saved_stats.defense - TRUE : 0)) >= lvl - TRUE || lvl == 1)
						saved_stats.defense = lvl
					. = TRUE
				if("Speed")
					var/lvl = clamp(text2num(params["level"]), 1, 5)
					if((points + (saved_stats.speed > TRUE ? saved_stats.speed - TRUE : 0)) >= lvl - TRUE || lvl == 1)
						saved_stats.speed = lvl
					. = TRUE
				if("Potential")
					var/lvl = clamp(text2num(params["level"]), 1, 5)
					if((points + (saved_stats.potential > TRUE ? saved_stats.potential - TRUE : 0)) >= lvl - TRUE || lvl == 1)
						saved_stats.potential = lvl
					. = TRUE
				if("Range")
					var/lvl = clamp(text2num(params["level"]), 1, 5)
					if((points + (saved_stats.range > TRUE ? saved_stats.range - TRUE : 0)) >= lvl - TRUE || lvl == 1)
