	...
	if(mob && !isdead(mob))
		...
		if(alert_admins)
			...
			if(admins_are_going_to_kill_you)
				message_admins("[ADMIN_LOOKUPFLW(usr)] might have attempted to exploit the antagonist late awaken system.")
				log_admin("[ADMIN_LOOKUP(usr)] might have attempted to exploit the antagonist late awaken system.")
		else
