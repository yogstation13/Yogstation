/datum/controller/subsystem/processing/quirks/proc/checkquirks(mob/living/user,client/cli) // Returns true when the player isn't trying to fuckin scum the mood pref stuff to exploit
	...
	U.mood_enabled = cli.prefs.read_preference(/datum/preference/toggle/quiet_mode) // Marks whether this player had moods enabled in preferences at the time of spawning (helps prevent exploitation)
	
	var/ismoody = (!CONFIG_GET(flag/disable_human_mood) || (cli.prefs.read_preference(/datum/preference/toggle/quiet_mode))) // If moods are globally enabled, or this guy does indeed have his mood pref set to Enabled
