...
/obj/item/storage/backpack/holding/build_worn_icon(var/state = "", var/default_layer = 0, var/default_icon_file = null, var/isinhands = FALSE, var/femaleuniform = NO_FEMALE_UNIFORM)
	state = item_state
	if(default_icon_file == DEFAULTFILE_HEAD)
		sprite_sheets_file = "head.dmi" // thats a fun dilemma.... how to keep the tg sprites when doing back but do yogs sprites when worn on head.
	else
		sprite_sheets_file = initial(sprite_sheets_file)
	return ..()
