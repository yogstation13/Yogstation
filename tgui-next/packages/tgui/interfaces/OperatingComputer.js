...
export const OperatingComputer = (props) => {
  ...
  const { table, surgeries = [], procedures = [], patient = {} } = data;
  return (
    <Tabs>
      <Tabs.Tab key="state" label="Patient State">
        {!table && <NoticeBox>No Table Detected</NoticeBox>}
        <Section>
          <Section title="Patient State" level={2}>
            {patient ? (
              <LabeledList>
                ...
                <LabeledList.Item label="Health">
                  <ProgressBar
                    ...
                    content={<AnimatedNumber value={patient.health} />}
                  />
                </LabeledList.Item>
                {damageTypes.map((type) => (
                  <LabeledList.Item key={type.type} label={type.label}>
                    <ProgressBar
                      ...
                      color="bad"
                      content={<AnimatedNumber value={patient[type.type]} />}
                    />
                  </LabeledList.Item>
                ))}
              </LabeledList>
          ...
          <Section title="Initiated Procedures" level={2}>
            {procedures.length
      ...
      <Tabs.Tab key="procedures" label="Surgery Procedures">
