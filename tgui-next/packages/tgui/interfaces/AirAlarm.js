...
export const AirAlarm = (props) => {
  ...
  return (
    <Fragment>
      ...
      {!locked && <AirAlarmControl state={state} />}
    </Fragment>
  );
};
...
const AirAlarmControlVents = (props) => {
  ...
  return vents.map((vent) => (
    <Vent key={vent.id_tag} state={state} {...vent} />
  ));
};
...
const AirAlarmControlScrubbers = (props) => {
  ...
  return scrubbers.map((scrubber) => (
    <Scrubber key={scrubber.id_tag} state={state} {...scrubber} />
  ));
};

const Scrubber = (props) => {
  const { long_name, power, scrubbing, id_tag, widenet, filter_types } = props;
  const { act } = useBackend(props);
  return (
    <Section
      level={2}
