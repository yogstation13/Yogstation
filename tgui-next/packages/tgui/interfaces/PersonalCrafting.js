...
export const PersonalCrafting = (props) => {
  ...
  const craftingRecipes = map((subcategory, category) => {
    ...
    const firstSubcatName = Object.keys(subcategory).find(
      (name) => name !== "has_subcats"
    );
    return {
      category,
      subcategory,
  ...
  const busyBox = !!busy && (
    <Dimmer fontSize="40px" textAlign="center">
      <Box mt={30}>
        ...
        <Tabs>
          {craftingRecipes.map((recipe) => (
            ...
            >
              {() =>
                (!recipe.hasSubcats && (
                  <CraftingList craftables={recipe.subcategory} state={state} />
                )) || (
                  <Tabs vertical>
                    {map((value, name) => {
                      if (name === "has_subcats") {
