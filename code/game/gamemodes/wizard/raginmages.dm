...
/datum/game_mode/wizard/raginmages/proc/make_more_mages()
	...
	spawn(rand(spawn_delay_min, spawn_delay_max))
		...
		candidates = pollGhostCandidates("Do you wish to be considered for the position of Space Wizard Foundation 'diplomat'?", banlist, src, bullshit ? ROLE_BULLSHITMAGES : ROLE_RAGINMAGES, ignore_category = POLL_IGNORE_RAGINMAGES)
		if(!candidates.len)
