...
/datum/game_mode/wizard/raginmages/proc/make_more_mages()
	...
	spawn(rand(spawn_delay_min, spawn_delay_max))
		...
		candidates = pollGhostCandidates("Do you wish to be considered for the position of Space Wizard Federation 'diplomat'?", banlist, src, bullshit_mode ? ROLE_BULLSHITMAGES : ROLE_RAGINMAGES : ROLE_WIZARD, ignore_category = POLL_IGNORE_RAGINMAGES)
		if(!candidates.len)
