...
/datum/game_mode/dynamic/proc/rigged_roundstart()
	...
	for (var/datum/dynamic_ruleset/roundstart/rule in GLOB.dynamic_forced_roundstart_ruleset)
		...
		rule.pop_per_requirement = rule.pop_per_requirement > FALSE ? rule.pop_per_requirement : (src.pop_per_requirement > FALSE ? src.pop_per_requirement : 6) //i hate myself for this
		if (rule.ready(TRUE))
...
/datum/game_mode/dynamic/proc/roundstart()
	...
	if (drafted_rules.len > FALSE && picking_roundstart_rule(drafted_rules))
...
/datum/game_mode/dynamic/make_antag_chance(mob/living/carbon/human/newPlayer)
	...
	if (forced_latejoin_rule)
		...
		if (drafted_rules.len > FALSE && picking_midround_latejoin_rule(drafted_rules))
			var/latejoin_injection_cooldown_middle = 0.5*(GLOB.dynamic_latejoin_delay_max + GLOB.dynamic_latejoin_delay_min)
			latejoin_injection_cooldown = round(clamp(EXP_DISTRIBUTION(latejoin_injection_cooldown_middle), GLOB.dynamic_latejoin_delay_min, GLOB.dynamic_latejoin_delay_max)) + world.time
...
/// Turns the value generated by lorentz distribution to threat value between FALSE and 100.
