...
/obj/machinery/newscaster/Topic(href, href_list)
	...
	if ((usr.contents.Find(src) || ((get_dist(src, usr) <= 1) && isturf(loc))) || issilicon(usr))
		...
		if(href_list["set_channel_name"])
				...
				if(choice=="Confirm")
					...
					if(input_param==1)          //If input_param == 1 we're submitting a new wanted issue. At 2 we're just editing an existing one.
						...
						if(GLOB.news_network.wanted_issue.isAdminMsg)
							tgui_alert(usr, "The wanted issue has been distributed by a Nanotrasen higherup. You cannot edit it.","Ok")
							return
			...
			if(GLOB.news_network.wanted_issue.isAdminMsg)
				tgui_alert(usr, "The wanted issue has been distributed by a Nanotrasen higherup. You cannot take it down.","Ok")
				return
			...
			if(FC.is_admin_channel)
				tgui_alert(usr, "This channel was created by a Nanotrasen Officer. You cannot censor it.","Ok")
				return
			...
			if(MSG.is_admin_message)
				tgui_alert(usr, "This message was created by a Nanotrasen Officer. You cannot censor its author.","Ok")
				return
			...
			if(FC.is_admin_channel)
				tgui_alert(usr, "This channel was created by a Nanotrasen Officer. You cannot place a D-Notice upon it.","Ok")
				return
