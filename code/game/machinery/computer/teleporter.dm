...
/obj/machinery/computer/teleporter/ui_act(action, params)
	...
	switch(action)
		if("regimeset")
			...
			power_station.teleporter_hub.update_appearance(updates = ALL)
		...
		if("calibrate")
			...
			calibrating = TRUE
			power_station.update_appearance(updates = ALL)
...
/obj/machinery/computer/teleporter/proc/set_target(mob/user)
	...
	if(regime_set == "Teleporter")
		...
		if(target_station.teleporter_hub)
			...
			target_station.teleporter_hub.update_appearance(updates = ALL)
		if(target_station.teleporter_console)
			target_station.teleporter_console.stat &= ~NOPOWER
			target_station.teleporter_console.update_appearance(updates = ALL)
