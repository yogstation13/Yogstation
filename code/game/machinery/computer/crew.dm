...
/datum/crewmonitor/proc/update_data(z)
	...
	for(var/mob/living/carbon/human/H in GLOB.carbon_list)
		...
		if ((H.z == FALSE || H.z == z) && (istype(H.w_uniform, /obj/item/clothing/under) || nanite_sensors))
			U = H.w_uniform

			// Are the suit sensors on?
			if (nanite_sensors || ((U.has_sensor > 0) && U.sensor_mode))
				pos = H.z == FALSE || (nanite_sensors || U.sensor_mode == SENSOR_COORDS) ? get_turf(H) : null

				// Special case: If the mob is inside an object confirm the z-level on turf level.
				if (H.z == FALSE && (!pos || pos.z != z))
					continue
