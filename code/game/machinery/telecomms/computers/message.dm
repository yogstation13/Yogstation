	...
	if(usr.contents.Find(src) || (in_range(src, usr) && isturf(loc)) || issilicon(usr))
		...
		if (href_list["auth"])
			if(LINKED_SERVER_NONRESPONSIVE)
				...
				var/dkey = stripped_input(usr, "Please enter the decryption key.") as text|null)
				if(dkey && dkey != "")
		...
		if (href_list["pass"])
			if(LINKED_SERVER_NONRESPONSIVE)
				...
				if(dkey && dkey != "")
					if(linkedServer.decryptkey == dkey)
						var/newkey = stripped_input(input(usr,"Please enter the new key (3 - 16 characters max):"))
						if(length(newkey) <= 3)
