		...
		if(do_after(user, 0.5 SECONDS, H)) //Counting the delay for "Clear", revive time is 5sec total
			...
			if(H.stat == DEAD)
				...
				if (H.suiciding)
					...
					failed = span_boldnotice("[req_defib ? "[defib]" : "[src]"] buzzes: Resuscitation failed - [isipc(H) ? "Patient's biology is a positronic chassis. Reboot board usage as opposed to defibrillating shocks may prove successful." : "Biology incompatible."]")
