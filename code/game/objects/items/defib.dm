		...
		if(do_after(user, 0.5 SECONDS, H)) //Counting the delay for "Clear", revive time is 5sec total
			...
			if(H.stat == DEAD)
				...
				if (H.suiciding)
					...
					failed = span_boldnotice("[req_defib ? "[defib]" : "[src]"] buzzes: Resuscitation failed -  Severe [isipc(H) ? "chassis" : "tissue"] damage makes recovery of patient impossible via defibrillator [isipc(H) ? "or reboot board. Mechanical repairs via tools may result in successful operation." : ". Surgical repair may allow for successful resuscitation.")
				...
				else if (HAS_TRAIT(H, TRAIT_NODEFIB) & isipc(H))
					failed = span_boldnotice("[req_defib ? "[defib]" : "[src]"] buzzes: Resuscitation failed - Patient's biology is a positronic chassis. Reboot board usage as opposed to defibrillating shocks may prove successful.")
