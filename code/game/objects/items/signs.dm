...
/obj/item/picket_sign/attack_self(mob/living/carbon/human/user)
	...
	if(NSCOMPONENT(user.dir)) //So signs are waved horizontally relative to what way the player waving it is facing.
		animate(user, pixel_x = user.pixel_x + (1 * direction)*PIXEL_MULTIPLIER, time = 1, easing = SINE_EASING)
		animate(pixel_x = user.pixel_x - (2 * direction)*PIXEL_MULTIPLIER, time = 1, easing = SINE_EASING)
		animate(pixel_x = user.pixel_x + (2 * direction)*PIXEL_MULTIPLIER, time = 1, easing = SINE_EASING)
		animate(pixel_x = user.pixel_x - (2 * direction)*PIXEL_MULTIPLIER, time = 1, easing = SINE_EASING)
		animate(pixel_x = user.pixel_x + (1 * direction)*PIXEL_MULTIPLIER, time = 1, easing = SINE_EASING)
	else
		animate(user, pixel_y = user.pixel_y + (1 * direction)*PIXEL_MULTIPLIER, time = 1, easing = SINE_EASING)
		animate(pixel_y = user.pixel_y - (2 * direction)*PIXEL_MULTIPLIER, time = 1, easing = SINE_EASING)
		animate(pixel_y = user.pixel_y + (2 * direction)*PIXEL_MULTIPLIER, time = 1, easing = SINE_EASING)
		animate(pixel_y = user.pixel_y - (2 * direction)*PIXEL_MULTIPLIER, time = 1, easing = SINE_EASING)
		animate(pixel_y = user.pixel_y + (1 * direction)*PIXEL_MULTIPLIER, time = 1, easing = SINE_EASING)
