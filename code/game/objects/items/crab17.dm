...
/obj/structure/checkoutmachine/examine(mob/living/user)
	. = ..()
	. += span_info("Its integrated integrity meter reads: <b>HEALTH: [atom_integrity]</b>.")
...
/obj/structure/checkoutmachine/attackby(obj/item/W, mob/user, params)
	...
	if(istype(W, /obj/item/card/id))
		...
		if(do_after(user, 4 SECONDS, src))
			if(!card.registered_account.being_dumped)
				return
			to_chat(user, span_warning("You quickly cash out your funds to a more secure banking location. Funds are safe."))
