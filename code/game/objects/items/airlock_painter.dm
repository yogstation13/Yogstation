...
/obj/item/airlock_painter/floor_painter/afterattack(atom/target, mob/user, proximity)
	. = ..()
	var/turf/open/floor/F = target
	if(!proximity)
		to_chat(user, span_notice("You need to get closer!"))
...
/obj/item/airlock_painter/floor_painter/Initialize(mapload)
	. = ..()
	ink = new /obj/item/toner/large(src)
...
/obj/item/airlock_painter/floor_painter/ui_act(action,list/params)
	. = ..()
	if(.)
		return
