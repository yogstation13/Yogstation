...
/obj/item/examine(mob/user) //This might be spammy. Remove?
	...
	if(block_upgrade_walk)
		. += "[src] appears strong enough you'd be able to block [block_level ? "a wider area " : ""] with it if you were walking."
...
/obj/item/proc/on_block(mob/living/carbon/human/owner, atom/movable/hitby, attack_text = "the attack", damage = 0, attack_type = MELEE_ATTACK)
	...
	if(owner.getStaminaLoss() >= 35 && HAS_TRAIT(src, TRAIT_NODROP))//if you don't drop the item, you can't block for a few seconds
		owner.blockbreak()
	return TRUE
