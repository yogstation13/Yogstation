	...
	if(open)
		...
		if(I.w_class + space <= maxspace)
			...
			if(!user.transferItemToLoc(I, src))
				to_chat(user, span_warning("\The [I] is stuck to your hand, you cannot put it in the safe!</span>"))
				return
			to_chat(user, span_notice("You put [I] in [src].</span>"))
		else
			to_chat(user, span_warning("[I] won't fit in [src].</span>"))
	else
		if(istype(I, /obj/item/clothing/neck/stethoscope))
			attack_hand(user)
			return
		else
			to_chat(user, span_warning("You can't put [I] into the safe while it is closed!</span>"))
			return
	...
	switch(action)
		if("open")
			if(!check_unlocked() && !open && !broken)
				to_chat(user, span_warning("You cannot open [src], as its lock is engaged!</span>"))
				return
...
/obj/structure/safe/proc/check_unlocked()
	...
	if(current_tumbler_index > number_of_tumblers)
		locked = FALSE
		visible_message(span_boldnotice("[pick("Spring", "Sprang", "Sproing", "Clunk", "Krunk")]!</span>"))
		return TRUE
	locked = TRUE
	return FALSE
...
/obj/structure/safe/proc/notify_user(user, canhear, sounds, total_ticks, current_tick)
	if(!canhear)
		return
	if(current_tick == 2)
		to_chat(user, span_italics("The sounds from [src] are too fast and blend together.</span>"))
	if(total_ticks == 1 || prob(SOUND_CHANCE))
		to_chat(user, span_italics("You hear a [pick(sounds)] from [src].</span>"))
