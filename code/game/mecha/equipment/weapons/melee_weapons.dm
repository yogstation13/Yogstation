...
/obj/item/mecha_parts/mecha_equipment/melee_weapon/afterburner
	...
	weapon_damage = 0 // damage is based on mech punch damage * step time / 2
	minimum_damage = 10
	structure_damage_mult = 4
	equip_actions = list(/datum/action/cooldown/mecha_afterburner)

/obj/item/mecha_parts/mecha_equipment/melee_weapon/afterburner/can_attach(obj/mecha/new_mecha)
	if(istype(new_mecha, /obj/mecha/working/clarke) && new_mecha.equipment.len < new_mecha.max_equip) // clarke because it's fucking COOL
		return TRUE
	return ..()

/obj/item/mecha_parts/mecha_equipment/melee_weapon/afterburner/attach(obj/mecha/new_mecha)
	. = ..()
...
/obj/item/mecha_parts/mecha_equipment/melee_weapon/afterburner/detach(atom/moveto)
	UnregisterSignal(chassis, list(COMSIG_MOVABLE_PRE_ENTER, COMSIG_MOVABLE_MOVED))
	return ..()

/obj/item/mecha_parts/mecha_equipment/melee_weapon/afterburner/proc/headstomp(atom/movable/source, atom/entering_loc)
	if(!chassis.throwing)
		return FALSE
