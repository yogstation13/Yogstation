...
#define isnightmare(A) (A?.mind?.has_antag_datum(/datum/antagonist/nightmare)) //midround antag
#define is_team_darkspawn(A) ((A.mind && isdarkspawn(A) || isthrall(A)) || ispsyche(A) || isnightmare(A) || (ROLE_DARKSPAWN in A.faction)) //also checks factions, so things can be immune to darkspawn spells without needing an antag datum
