// Flags for the obj_flags var on /obj				(1<<8)
...
#define BEING_SHOCKED			(1<<11) // Whether this thing is currently (already) being shocked by a tesla
...
// Flags for the item_flags var on /obj/item
#define UNCATCHABLE				(1<<20) // Makes any item uncatchable if it is thrown at them
#define MEDRESIST				(1<<21) // This item will block medical sprays when worn
...
///prevents you from being stupid if you shower in them
...
#define CLOTHING_SHREDDED	2 // The clothing is useless and cannot be equipped unless repaired first If it shouldn't play sounds the first time it lands, used for reverse mode
