...
/datum/controller/subsystem/dbcore/proc/InitializeRound()
	...
	var/datum/db_query/query_fix_connections = SSdbcore.NewQuery("UPDATE [format_table_name("connection_log")] SET `left` = NOW() WHERE `left` IS NULL AND round_id = :id", list("id" = text2num(GLOB.round_id) - 1))
