...
/datum/controller/subsystem/machines/proc/unregister_machine(obj/machinery/machine)
	...
	if(existing) //doesn't check if the list is populated, checks if the list is even a list at all (changed because proc maints creates and destroys machines before this subsystem initializes)
		existing -= machine
		if(!length(existing))
