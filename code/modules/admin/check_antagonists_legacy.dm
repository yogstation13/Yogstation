// This is the backup HTML version, for the TGUI version, see check_antagonists.dm
// All procs have been prefixed with "legacy_" except for is_gamemode_hero

//I wish we had interfaces sigh, and i'm not sure giving team and antag common root is a better solution here

//Name shown on antag list
/datum/antagonist/proc/legacy_antag_listing_name()
	if(!owner)
		return "Unassigned"
	if(owner.current)
...
/datum/antagonist/proc/legacy_antag_listing_status()
	if(!owner)
		return "(Unassigned)"
	if(!owner.current)
...
/datum/antagonist/proc/legacy_antag_listing_commands()
	if(!owner)
		return
	var/list/parts = list()
...
/datum/antagonist/proc/legacy_antag_listing_entry()
	var/list/parts = list()
	if(show_name_in_check_antagonists)
		parts += "[legacy_antag_listing_name()]([name])"
	else
		parts += legacy_antag_listing_name()
	parts += legacy_antag_listing_status()
	parts += legacy_antag_listing_commands()
	return "<tr><td>[parts.Join("</td><td>")]</td></tr>"


/datum/team/proc/legacy_get_team_antags(antag_type,specific = FALSE)
	. = list()
...
/datum/team/proc/legacy_antag_listing_entry()
	//NukeOps:
	// Jim (Status) FLW PM TP
...
/datum/team/proc/legacy_antag_listing_footer()
	return

/datum/team/proc/legacy_antag_listing_name()
	return name

/datum/admins/proc/legacy_build_antag_listing()
	...
	for(var/datum/team/T in all_teams)
		...
		if(T.is_gamemode_hero())
			priority_sections += T.legacy_antag_listing_entry()
		else
			sections += T.legacy_antag_listing_entry()
	...
	for(var/i in 1 to all_antagonists.len)
		...
		current_section += current_antag.legacy_antag_listing_entry() // Name - (Traitor) - FLW | PM | TP
...
/datum/admins/proc/legacy_check_antagonists()
	...
	dat += legacy_build_antag_listing()
