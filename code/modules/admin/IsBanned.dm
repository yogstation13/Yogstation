		...
		if(!SSdbcore.Connect())
			...
			if(!real_bans_only)
				...
				if(SSechelon.is_match(ckey, address))
					...
					key_cache[key] = FALSE
					return list("reason" = "proxy", "desc" = "Round ID [GLOB.round_id]: Your connection has been blocked due to using a VPN or proxy service. Disable your VPN product and reconnect. If this was done in error or if you are required to use a VPN to connect to the server, please visit [CONFIG_GET(string/banappeals) || "the forums"].")
