...
/datum/antagonist/proc/antag_panel_objectives()
	...
	if (objectives.len == 0)
		...
		for(var/datum/objective/objective in objectives)
			result += "<B>[obj_count]</B>: <font color=[objective.check_completion() ? "green" : "red"]>[objective.explanation_text][objective.check_completion() ? " (COMPLETED)" : ""]</font> <a href='byond://?src=[REF(owner)];obj_edit=[REF(objective)]'>Edit</a> <a href='byond://?src=[REF(owner)];obj_delete=[REF(objective)]'>Delete</a> <a href='byond://?src=[REF(owner)];obj_completed=[REF(objective)]'><font color=[objective.completed ? "green" : "red"]>[objective.completed ? "Mark as incomplete" : "Mark as complete"]</font></a><br>"
			obj_count++
...
/datum/mind/proc/traitor_panel()
	...
	if(quiet_round)
		out += "<font color=red><b>QUIET ROUND ACTIVE</b></font> (<a href='byond://?src=\ref[src];quiet_override=1'>Override</a>)<br>"
	// yogs end
