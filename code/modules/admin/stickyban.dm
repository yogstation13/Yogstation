/datum/admins/proc/stickyban(action,data)
	...
	switch (action)
		...
		if ("unexempt")
			...
			if (tgui_alert("Are you sure you want to unexempt [alt] from [ckey]'s sticky ban?","Are you sure",list("Yes","No")) == "No")
				return
		...
		if ("revert")
			if (!data["ckey"])
				return
			var/ckey = data["ckey"]
			if (tgui_alert("Are you sure you want to revert the sticky ban on [ckey] to its state at round start (or last edit)?","Are you sure",list("Yes","No")) == "No")
				return
			var/ban = get_stickyban_from_ckey(ckey)
			if (!ban)
				to_chat(usr, span_adminnotice("Error: No sticky ban for [ckey] found!"), confidential=TRUE)
