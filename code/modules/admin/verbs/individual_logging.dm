/proc/show_individual_logging_panel(mob/M, source = LOGSRC_CLIENT, type = INDIVIDUAL_ATTACK_LOG)
	...
	if(M.client)
		...
		dat += " | " // yogs - LOOC log
		dat += individual_logging_panel_link(M, INDIVIDUAL_TOCHAT_LOG, LOGSRC_CLIENT, "TOCHAT Log", source, ntype)
		dat += " | "
	...
	dat += " | "
	dat += individual_logging_panel_link(M, INDIVIDUAL_LOOC_LOG, LOGSRC_MOB, "LOOC Log", source, ntype) // yogs - LOOC log
	dat += " | " // yogs - LOOC log
	dat += individual_logging_panel_link(M, INDIVIDUAL_TOCHAT_LOG, LOGSRC_MOB, "TOCHAT Log", source, ntype)
	dat += " | "
