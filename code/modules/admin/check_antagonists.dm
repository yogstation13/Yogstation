...
/datum/admins/proc/check_antagonists()
	...
	dat += "<a href='?_src_=holder;[HrefToken()];toggle_continuous=1'>[CONFIG_GET(keyed_list/continuous)[SSticker.mode.config_tag] ? "Continue if antagonists die" : "End on antagonist "]</a>"
	...
	if(CONFIG_GET(keyed_list/midround_antag)[SSticker.mode.config_tag])
		...
		dat += "If limits past: <a href='?_src_=holder;[HrefToken()];toggle_noncontinuous_behavior=1'>[SSticker.mode.round_ends_with_antag_ ? "End The Round" : "Continue As Extended"]</a><BR>"
