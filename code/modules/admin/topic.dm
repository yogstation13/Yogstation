	...
	/*else if(href_list["editmessageseverity"])
		...
		switch(alert("Temporary Ban for [M.key]?",,"Yes","No", "Cancel"))
			if("Yes")
				...
				create_message("note", ckey(M.ckey), usr.ckey, "Banned for [mins] minutes - [reason]", null, null, 0, 0, null, 0)
			...
			if("No")
				...
				create_message("note", ckey(M.ckey), usr.ckey, "Permanently banned - [reason]", null, null, 0, 0, null, 0)
				to_chat(M, "<span class='boldannounce'><BIG>You have been banned by [usr.client.key].\nReason: [reason]</BIG></span>")
