...
/obj/machinery/bci_implanter/attackby(obj/item/O, mob/user, params)
	. = ..()
	if (.)
		return .
...
/obj/machinery/bci_implanter/attackby(obj/item/A, mob/user, params)
	if (!occupant && default_deconstruction_screwdriver(user, icon_state, icon_state, A))
		update_appearance()
		return 

	if (default_pry_open(A, close_after_pry = FALSE, open_density = FALSE, closed_density = TRUE))
		return 
...
/obj/machinery/bci_implanter/close_machine(mob/living/carbon/user, density_to_set = TRUE)
	...
	var/mob/living/carbon/human/carbon_occupant = occupant
	if (istype(occupant))
		var/obj/item/organ/internal/cyberimp/bci/bci_organ = carbon_occupant.getorgan(/obj/item/organ/internal/cyberimp/bci)
