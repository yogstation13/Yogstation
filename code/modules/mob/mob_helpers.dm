...
/proc/is_special_character(mob/potentially_special_mob)
	if(!SSticker.HasRoundStarted())
		return FALSE
	if(!istype(potentially_special_mob))
		return FALSE
	if(iscyborg(potentially_special_mob)) //as a borg you're now beholden to your laws rather than greentext
		return FALSE
	if(isAI(potentially_special_mob))
		var/mob/living/silicon/ai/potentially_special_ai = potentially_special_mob
		return (potentially_special_ai.laws?.zeroth && (potentially_special_ai.mind?.special_role || !isnull(potentially_special_ai.mind?.antag_datums)))
	if(potentially_special_mob.mind?.special_role || !isnull(potentially_special_mob.mind?.antag_datums)) //they have an antag datum!
		return TRUE
	return FALSE
