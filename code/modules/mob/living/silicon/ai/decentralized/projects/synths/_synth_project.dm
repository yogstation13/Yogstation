...
/datum/synth_project/New(new_synth, new_os)
	synth = new_synth
	os = new_os
	if(!synth || !os)
		qdel(src)
...
/datum/synth_project/proc/canResearch()
	if(!research_requirements)
		return TRUE
	for(var/P in research_requirements)
		if(!os.has_completed_project(P))
			return FALSE
	return TRUE

/datum/synth_project/proc/run_project(force_run = FALSE)
	...
	if(!force_run)
		if(!canRun())
			return FALSE
	running = TRUE
	os.running_projects += src
	return TRUE


/datum/synth_project/proc/stop()
	SHOULD_CALL_PARENT(TRUE)
	running = FALSE
	os.running_projects -= src
	return TRUE
	
//Important! This isn't for checking processing requirements. That is checked on the synth os for ease of references (See synth_os.dm). This is just for special cases (Like uhh, not wanting the program to run while X runs or similar)
/datum/synth_project/proc/canRun()
	SHOULD_CALL_PARENT(TRUE)
	return !running

//Run when project is finished. For passive upgrades or adding abilities.
/datum/synth_project/proc/finish()
	return

/datum/synth_project/proc/add_ability(datum/action/innate/ai/ability)
	var/datum/action/innate/ai/has_ability = locate(ability) in synth.actions
	if(has_ability)
		return FALSE
