...
/mob/living/silicon/bullet_act(obj/projectile/Proj, def_zone)
	...
	if((Proj.damage_type == BRUTE || Proj.damage_type == BURN))
		var/damage = run_armor(Proj.damage * (demolition_factor ? (Proj.demolition_mod + demolition_factor - 1) / demolition_factor : 1), Proj.damage_type, Proj.armor_flag, Proj.armour_penetration)
		adjustBruteLoss(damage)
		if(prob(damage*1.5))
			for(var/mob/living/M in buckled_mobs)
