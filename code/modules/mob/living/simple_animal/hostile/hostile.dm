...
//These vars are related to how mobs locate and target
	...
	var/list/unwanted_objects = list(/turf) //Like wanted_objects, but for objects that are not allowed to be attacked. Usefull to deal with subtypes of something in wanted_objects.
...
/mob/living/simple_animal/hostile/CanAttack(atom/the_target)//Can we actually attack a possible target?
	//Yogs - removed isturf check, moved to unwanted_objects
	if(!the_target || the_target.type == /atom/movable/lighting_object) // bail out on invalids
		return FALSE
	
	if(is_type_in_typecache(the_target,unwanted_objects))
		return FALSE
