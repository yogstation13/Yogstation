...
/mob/living/simple_animal/hostile/proc/ListTargets()//Step 1, find out what we can see
	if(!search_objects)
		...
		var/static/hostile_machines = typecacheof(list(/obj/machinery/porta_turret, /obj/mecha, /obj/structure/clockwork/ocular_warden, /obj/spacepod)) // yogs - add spacepod
...
/mob/living/simple_animal/hostile/CanAttack(atom/the_target)//Can we actually attack a possible target?
	...
	if(search_objects < 2)
		...
		if(istype(the_target, /obj/structure/clockwork/ocular_warden))
			var/obj/structure/clockwork/ocular_warden/OW = the_target
			if(OW.target != src)
				return FALSE
			return TRUE
	if(isobj(the_target))
