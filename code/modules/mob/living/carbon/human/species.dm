...
/datum/species/proc/on_species_loss(mob/living/carbon/human/C, datum/species/new_species, pref_load)
	...
	C.remove_movespeed_modifier(MOVESPEED_ID_SPECIES) // Remove any slow down from the cold
...
/datum/species/proc/handle_body_temperature(mob/living/carbon/human/H)
	...
	if(H.bodytemperature > bodytemp_heat_damage_limit && !HAS_TRAIT(H, TRAIT_RESISTHEAT))
		...
		H.apply_damage(burn_damage, BURN) // Apply the damage to all body parts
	...
	else // We are not to hot or cold, remove status and moods
		H.clear_alert("temp")
...
/datum/species/proc/handle_environment_pressure(datum/gas_mixture/environment, mob/living/carbon/human/H)
	...
	switch(adjusted_pressure)
		...
		// Very low pressure, show an alert and take damage
			...
			if(HAS_TRAIT(H, TRAIT_RESISTLOWPRESSURE)) // We have low pressure resit trait, clear alerts
				H.clear_alert("pressure")
			else
