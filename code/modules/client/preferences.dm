...
/datum/preferences/New(client/C)
	...
	if(istype(C))
		if(!IsGuestKey(C.key))
			...
			else if(is_donator(C) || (C.ckey in get_donators()) || (C.ckey in GLOB.mentor_datums)) // the Latter handles race cases where the prefs are not fully loaded in, or GLOB.donators hasn't loaded in yet, or GLOB.mentor_datums hasn't loaded in yet
