...
/datum/preferences/proc/copy_to(mob/living/carbon/human/character, icon_updates = 1, roundstart_checks = TRUE)
	...
	if(roundstart_checks && !(pref_species.id in GLOB.roundstart_races) && (!(pref_species.id in GLOB.donator_races) && !is_donator(character))  && !(pref_species.id in (CONFIG_GET(keyed_list/roundstart_no_hard_check))))
