...
/datum/preferences/New(client/C)
	...
	if(istype(C))
		if(!IsGuestKey(C.key))
			...
			else if(is_donator(C) || (C.ckey in get_donators()) || C.is_mentor()) // the Latter handles race cases where the prefs are not fully loaded in, or GLOB.donators hasn't loaded in yet
