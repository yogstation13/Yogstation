...
//////////////////////////////////////////////////////////////////
//		Fish Tanks(Ported from Paradase by SuperSlayer)			//
//////////////////////////////////////////////////////////////////
...
/obj/machinery/fishtank/proc/spill_water()
	...
	switch(tank_type)
		...
		else if("tank")										//Fishtank: This SHOULD wets it's own tile and the 4 adjacent tiles (cardinal directions). But it wouldn't, because i do not know how to make this.
			if(istype(T))
				T.MakeSlippery(TURF_WET_WATER, min_wet_time = 10, wet_time_to_add = 5)
		else if("wall")								//Wall-tank: Wets it's own tile and the surrounding 8 tiles (3x3 square)
			for(var/turf/open/Y in spiral_range_turfs(1, loc))
				Y.MakeSlippery(TURF_WET_WATER, min_wet_time = 15, wet_time_to_add = 5)
...
/obj/machinery/fishtank/attackby(obj/item/O, mob/user)
	...
	if(O.is_drainable())
		...
		if(O.reagents.total_volume)
			...
			water_value += O.reagents.get_reagent_amount(/datum/reagent/water/holywater) *1.1		//Holywater is (somehow) better. Who said religion had to make sense?
			water_value += O.reagents.get_reagent_amount(/datum/reagent/consumable/tonic) * 0.25		//Tonic water is 25% value
			water_value += O.reagents.get_reagent_amount(/datum/reagent/consumable/sodawater) * 0.50	//Sodawater is 50% value
			water_value += O.reagents.get_reagent_amount(/datum/reagent/toxin/fishwater) * 0.75	//Fishwater is 75% value, to account for the fish poo
			water_value += O.reagents.get_reagent_amount(/datum/reagent/consumable/ice) * 0.80			//Ice is 80% value
			var/message = ""
		...
		//Empty containers will scoop out water, filling the container as much as possible from the water_level
			...
			if(!water_level)
				...
				if(water_level >= O.reagents.maximum_volume) //Enough to fill the container completely
					O.reagents.add_reagent(/datum/reagent/toxin/fishwater, O.reagents.maximum_volume)
					...
					O.reagents.add_reagent(/datum/reagent/toxin/fishwater, water_level)
