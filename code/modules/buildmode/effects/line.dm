...
/obj/effect/buildmode_line/New(client/C, atom/atom_a, atom/atom_b, linename)
	...
	var/x_offset = ((atom_b.x * WORLD_ICON_SIZE) + atom_b.pixel_x) - ((atom_a.x * WORLD_ICON_SIZE) + atom_a.pixel_x)
	var/y_offset = ((atom_b.y * WORLD_ICON_SIZE) + atom_b.pixel_y) - ((atom_a.y * WORLD_ICON_SIZE) + atom_a.pixel_y)

	var/matrix/mat = matrix()
	mat.Translate(0, 16*PIXEL_MULTIPLIER)
