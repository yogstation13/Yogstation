...
function linkify(parent, insertBefore, text) {
	var start = 0;
	var match;
	var regex = /(?:(?:https?:\/\/)|(?:www\.))(?:[^ ]*?\.[^ ]*?)+[-A-Za-z0-9+&@#\/%?=~_|$!:,.;()]+/gi;
...
function linkify_fallback(text) {
	var rex = /((?:<a|<iframe|<img)(?:.*?(?:src="|href=").*?))?(?:(?:https?:\/\/)|(?:www\.))+(?:[^ ]*?\.[^ ]*?)+[-A-Za-z0-9+&@#\/%?=~_|$!:,.;]+/gi;
			...
			if (el.childNodes[c].nodeType === 3) {
				...
				for (var w = 0; w < words.length; w++) {
					...
					for (var i = 0; i < opts.highlightTerms.length; i++) {
						...
						if (window.console) console.log(newWord);
					}
	...
	if (!handled) {
		...
		if (typeof Node === "undefined") {
			...
			for (var i = 0; i < to_linkify.length; ++i) {
				to_linkify[i].innerHTML = linkify_fallback(
					to_linkify[i].innerHTML
				);
			}
		} else {
...
function swap() {
	//Swap to darkmode
	if (opts.darkmode) {
		document.getElementById("sheetofstyles").href =
			"browserOutput_white.css";
		opts.darkmode = false;
...
function ehjaxCallback(data) {
	...
	if (data == "softPang") {
		...
		$("#adminMusic").prop("src", "");
	} else if (data == "stopLobbyMusic") {
		//yogs start - lobby music
...
$(function () {
	...
	if (document.defaultCharset) {
		runByond(
			"?_src_=chat&proc=encoding&encoding=" +
				escaper(document.defaultCharset)
		);
	}
	// yogs end
	...
	if (savedConfig.smusicVolume) {
		...
		sendVolumeUpdate();
		internalOutput(
			'<span class="internal boldnshit">Loaded music volume of: ' +
				savedConfig.smusicVolume +
				"</span>",
			"internal"
		);
	} else {
	...
	$("body").on("keydown", function (e) {
		...
		if (c.length == 0) {
			...
			runByond(
				"byond://winset?mapwindow.map.focus=true;mainwindow.input.text=" +
					c
			);
			return false;
		} else {
	...
	$("#saveLog").click(function (e) {
		...
		if (!window.Blob) {
			output(
				'<span class="big red">This function is only supported on IE 10+. Upgrade if possible.</span>',
				"internal"
			);
			return;
		}
