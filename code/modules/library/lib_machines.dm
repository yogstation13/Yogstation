...
/obj/machinery/computer/libraryconsole/Topic(href, href_list)
	...
	if(href_list["setauthor"])
		...
		if(newauthor)
			...
			author = null
	if(href_list["search"])
		SQLquery = "SELECT author, title, category, id FROM [format_table_name("library")] WHERE deleted = NULL AND "
		if(category == "Any")
...
/proc/load_library_db_to_cache()
	...
	var/datum/DBQuery/query_library_cache = SSdbcore.NewQuery("SELECT id, author, title, category FROM [format_table_name("library")] WHERE deleted = NULL")
	...
	if(href_list["targetid"])
		...
		if(cooldown > world.time)
			...
			var/datum/DBQuery/query_library_print = SSdbcore.NewQuery(
				"SELECT * FROM [format_table_name("library")] WHERE id=:id AND deleted = NULL",
				list("id" = id)
			)
