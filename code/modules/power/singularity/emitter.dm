...
/obj/machinery/power/emitter/Destroy()
	if(SSticker.IsRoundInProgress())
		...
		investigate_log("<font color='red'>deleted</font> at [AREACOORD(T)]", INVESTIGATE_SUPERMATTER)
	QDEL_NULL(sparks)
	return ..()
...
/obj/machinery/power/emitter/interact(mob/user)
	...
	if(state == EMITTER_WELDED)
		...
		if(!locked && allow_switch_interact)
			...
			investigate_log("turned [active ? "<font color='green'>ON</font>" : "<font color='red'>OFF</font>"] by [key_name(user)] at [AREACOORD(src)]", INVESTIGATE_SUPERMATTER)
			update_icon()
	...
	if(active == TRUE)
		if(!active_power_usage || surplus() >= active_power_usage)
			...
			if(!powered)
				...
				investigate_log("regained power and turned <font color='green'>ON</font> at [AREACOORD(src)]", INVESTIGATE_SUPERMATTER)
		else
			if(powered)
				powered = FALSE
				update_icon()
				investigate_log("lost power and turned <font color='red'>OFF</font> at [AREACOORD(src)]", INVESTIGATE_SINGULO)
				investigate_log("lost power and turned <font color='red'>OFF</font> at [AREACOORD(src)]", INVESTIGATE_SUPERMATTER)
