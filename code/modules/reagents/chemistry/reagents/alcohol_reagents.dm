...
/datum/reagent/consumable/ethanol/hippies_delight/on_mob_life(mob/living/carbon/M)
	...
	switch(current_cycle)
		...
		if(6 to 10)
			...
			M.set_drugginess_if_lower(45 SECONDS)
			if(prob(20))
				M.emote(pick("twitch","giggle"))
		if (11 to 200)
			M.set_jitter_if_lower(40 SECONDS)
			M.set_dizzy_if_lower(40 SECONDS)
			M.set_drugginess_if_lower(60 SECONDS)
			if(prob(30))
				M.emote(pick("twitch","giggle"))
		if(201 to INFINITY)
			M.set_jitter_if_lower(60 SECONDS)
			M.set_dizzy_if_lower(60 SECONDS)
			M.set_drugginess_if_lower(75 SECONDS)
			if(prob(40))
				M.emote(pick("twitch","giggle"))
			if(prob(30))
				M.adjustToxLoss(2, 0)
