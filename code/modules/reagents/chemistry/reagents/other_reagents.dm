...
/datum/reagent/water/holywater/reaction_mob(mob/living/exposed_mob, methods=TOUCH, reac_volume)
	if(IS_SERVANT_OF_RATVAR(exposed_mob))
		to_chat(exposed_mob, span_userdanger("A darkness begins to spread its unholy tendrils through your mind, purging the Justiciar's influence!"))
	return ..()

/datum/reagent/water/holywater/on_mob_life(mob/living/carbon/affected_mob)
	if(affected_mob.blood_volume)
		affected_mob.blood_volume += 0.1 // water is good for you!
	if(!data)
		data = list("misc" = 1)
	data["misc"]++
	affected_mob.adjust_jitter_up_to(4 SECONDS, 20 SECONDS)
	...
	if(data["misc"] >= 25)		// 10 units, 45 seconds @ metabolism 0.4 units & tick rate 1.8 sec
		affected_mob.adjust_stutter_up_to(4 SECONDS, 20 SECONDS)
		affected_mob.set_dizzy_if_lower(10 SECONDS)
		if(IS_CULTIST(affected_mob) && prob(20))
			...
			switch(pick("speech", "message", "emote"))
				...
				if("emote")
					affected_mob.visible_message(span_warning("[affected_mob] [pick("whimpers quietly", "shivers as though cold", "glances around in paranoia")]."))
	if(data["misc"] >= (1 MINUTES)) // 30 units, 135 seconds
		if(IS_CULTIST(affected_mob))
		...
		if(IS_SERVANT_OF_RATVAR(affected_mob))
			affected_mob.mind.remove_antag_datum(/datum/antagonist/clockcult)
		affected_mob.remove_status_effect(/datum/status_effect/jitter)
		affected_mob.remove_status_effect(/datum/status_effect/speech/stutter)
		holder.remove_reagent(type, volume)	// maybe this is a little too perfect and a max() cap on the statuses would be better??
		return
	if(ishuman(affected_mob) && is_vampire(affected_mob) && prob(80)) // Yogs Start
		...
		if(!V.get_ability(/datum/vampire_passive/full))
			switch(data)
				...
				if(13 to INFINITY)
					affected_mob.visible_message("<span class='danger'>[affected_mob] suddenly bursts into flames!<span>", span_userdanger("You suddenly ignite in a holy fire!"))
					...
					affected_mob.adjustFireLoss(3)        //Hence the other damages... ain't I a bastard? // Yogs End
	if(ishuman(affected_mob) && is_sinfuldemon(affected_mob) && prob(80))
		switch(data)
			if(1 to 4)
				to_chat(affected_mob, span_warning("Your unholy blood begins to burn as holy power creeps through you."))
				affected_mob.adjustFireLoss(1)
			if(5 to 10)
				to_chat(affected_mob, span_danger("The burning deepens and strengthens!"))
				affected_mob.adjustFireLoss(2)
			if(11 to 12)
				to_chat(affected_mob, span_danger("Your flesh itself begins to melt apart in agony!"))
				affected_mob.adjustFireLoss(3)
				affected_mob.emote("scream")
			if(13 to INFINITY)
				affected_mob.visible_message("<span class='danger'>[affected_mob] suddenly ignites in a brilliant flash of white!<span>", span_userdanger("You suddenly ignite in a holy fire!"))
				affected_mob.adjust_fire_stacks(3)
				affected_mob.ignite_mob()
				affected_mob.adjustFireLoss(4)
