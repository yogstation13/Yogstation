...
/obj/machinery/chem_master/adjust_item_drop_location(atom/movable/AM) // Special version for chemmasters and condimasters
	if (AM == beaker)
		AM.pixel_x = -8*PIXEL_MULTIPLIER
		AM.pixel_y = 8*PIXEL_MULTIPLIER
		return null
	else if (AM == bottle)
		if (length(bottle.contents))
			AM.pixel_x = -13*PIXEL_MULTIPLIER
		else
			AM.pixel_x = -7*PIXEL_MULTIPLIER
		AM.pixel_y = -8*PIXEL_MULTIPLIER
		return null
	else
		var/md5 = md5(AM.name)
		for (var/i in 1 to 32)
			. += hex2num(md5[i])
		. = . % 9
		AM.pixel_x = ((.%3)*6)*PIXEL_MULTIPLIER
		AM.pixel_y = (-8 + (round( . / 3)*8))*PIXEL_MULTIPLIER
