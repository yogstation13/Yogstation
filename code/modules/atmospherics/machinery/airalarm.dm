...
/obj/machinery/airalarm/ui_data(mob/user)
	...
	for(var/gas_id in environment.get_gases())
		...
		cur_tlv = TLV[gas_id]
		data["environment_data"] += list(list(
								"name" = global.meta_gas_info[gas_id][META_GAS_NAME],
	...
	if(!locked || user.has_unlimited_silicon_privilege)
		...
		for(var/gas_id in global.meta_gas_info)
			if(!(gas_id in TLV)) // We're not interested in this gas, it seems.
				continue
			selected = TLV[gas_id]
			thresholds += list(list("name" = global.meta_gas_info[gas_id][META_GAS_NAME], "settings" = list()))
