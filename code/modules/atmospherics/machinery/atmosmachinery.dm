...
/obj/machinery/atmospherics/proc/getpipeimage(iconset, iconstate, direction, col=rgb(255,255,255), piping_layer=2, trinary = FALSE)
	...
	if((!(. = GLOB.pipeimages[identifier])))
		...
		if(trinary == TRUE && (piping_layer == 0 || piping_layer == 4))
