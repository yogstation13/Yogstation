...
/obj/machinery/atmospherics/components/binary/hypertorus/core/process_atmos()
	...
	if(linked_input.airs[1].get_moles(/datum/gas/hydrogen) > 0)
		var/fuel_injected = min(linked_input.airs[1].get_moles(/datum/gas/hydrogen), fuel_injection_rate * 0.1)
		linked_input.airs[1].adjust_moles(/datum/gas/hydrogen, -fuel_injected)
		internal_fusion.adjust_moles(/datum/gas/hydrogen, fuel_injected)
	if(linked_input.airs[1].get_moles(/datum/gas/tritium) > 0)
		var/fuel_injected = min(linked_input.airs[1].get_moles(/datum/gas/tritium), fuel_injection_rate * 0.1)
		linked_input.airs[1].adjust_moles(/datum/gas/tritium, -fuel_injected)
		internal_fusion.adjust_moles(/datum/gas/tritium, fuel_injected)
...
//7 reworks
	...
	//m_proto_nitrate = moderator_internal.get_moles(/datum/gas/pluonium)
			...
			switch(power_level)
				if(1)
					...
					if(m_bz > 150)
						...
						internal_output.adjust_moles(/datum/gas/pluonium, scaled_production * 0.25)
					...
					if(m_bz > 100)
						...
						for(var/mob/living/carbon/human/l in view(src, HALLUCINATION_RANGE(heat_output))) // If they can see it without mesons on.  Bad on them.
							if(!istype(l.glasses, /obj/item/clothing/glasses/meson))
								...
								l.hallucination = clamp(l.hallucination, 0, 200)
						moderator_internal.adjust_moles(/datum/gas/pluonium, scaled_production * 1.25)
	...
	if(waste_remove && power_level <= 5)
		...
		if(!ispath(filter_type))
			if(filter_type)
				...
				filtering = FALSE
		if(filtering && moderator_internal.get_gases()[filter_type])
		...
		if(internal_fusion.get_moles(/datum/gas/helium) > 0)
			internal_remove = internal_fusion.adjust_moles(/datum/gas/helium, -(internal_fusion.gases[/datum/gas/helium][MOLES] * 0.5))
			linked_output.airs[1].merge(internal_remove)
		if(internal_fusion.get_moles(/datum/gas/antinoblium) > 0)
			internal_remove = internal_fusion.adjust_moles(/datum/gas/antinoblium, -(internal_fusion.gases[/datum/gas/antinoblium][MOLES] * 0.05))
			linked_output.airs[1].merge(internal_remove)
...
/obj/item/paper/guides/jobs/atmos/hypertorus
	...
	use more advanced guides to understand how the various gases will act as moderators."
