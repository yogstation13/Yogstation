	...
	if(air1.total_moles())
		...
		if(beaker)
			if(reagent_transfer == 0) // Magically transfer reagents. Because cryo magic.
				...
				air1.adjust_moles(/datum/gas/plasma, -max(0,air1.get_moles(/datum/gas/plasma) - 2 / efficiency)) //Let's use gas for this
...
/obj/machinery/atmospherics/components/unary/cryo_cell/process_atmos()
	...
	if(!nodes[1] || !airs[1] || air1.get_moles(/datum/gas/plasma) < 5) // Turn off if the machine won't work.
		on = FALSE
		update_icon()
		return
	...
	if(occupant)
		...
		air1.set_moles(/datum/gas/plasma, max(0,air1.get_moles(/datum/gas/plasma) - 0.5 / efficiency)) // Magically consume gas? Why not, we run on cryo magic.
