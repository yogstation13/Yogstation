...
/obj/effect/appearance_clone/New(loc, atom/A)			//Intentionally not Initialize(mapload), to make sure the clone assumes the intended appearance in time for the camera getFlatIcon.
	if(istype(A))
...
/obj/item/camera/proc/camera_get_icon(list/turfs, turf/center, psize_x = 96, psize_y = 96, datum/turf_reservation/clone_area, size_x, size_y, total_x, total_y)
	...
	if(istype(clone_area) && total_x == clone_area.width && total_y == clone_area.height && size_x >= 0 && size_y > 0)
		var/cloned_center_x = round(clone_area.bottom_left_turfs[1] + ((total_x - 1) / 2))
		var/cloned_center_y = round(clone_area.bottom_left_turfs[2] + ((total_y - 1) / 2))
		for(var/t in turfs)
			var/turf/T = t
