...
/obj/item/photo/suicide_act(mob/living/carbon/user)
	user.visible_message(span_suicide("[user] is taking one last look at \the [src]! It looks like [user.p_theyre()] giving in to !"))//when you wanna look at photo of waifu one last time before you die...
	if (user.gender == MALE)
