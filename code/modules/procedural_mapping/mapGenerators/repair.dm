...
/datum/mapGeneratorModule/reload_station_map/generate()
	...
	for (var/path in SSmapping.config.GetFullMapPaths())
		var/datum/parsed_map/parsed = load_map(file(path), 1, 1, z_offset, measure_only = FALSE, no_changeturf = FALSE, crop_map = TRUE, x_lower = mother1.x_low, y_lower = mother1.y_low, x_upper = mother1.x_high, y_upper = mother1.y_high)
