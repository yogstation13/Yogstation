...
/datum/computer_file/program/ntnet_dos/ui_act(action, params)
	...
	switch(action)
		if("PRG_target_relay")
			for(var/obj/machinery/ntnet_relay/R in SSmodular_computers.station_network.relays)
		...
		if("PRG_execute")
			if(target)
				...
				if(SSmodular_computers.station_network.intrusion_detection_enabled)
					var/obj/item/computer_hardware/network_card/network_card = computer.all_components[MC_NET]
					SSmodular_computers.station_network.add_log("IDS WARNING - Excess traffic flood targeting relay [target.uid] detected from device: [network_card.get_network_tag()]")
					SSmodular_computers.station_network.intrusion_detection_alarm = TRUE
			return TRUE
