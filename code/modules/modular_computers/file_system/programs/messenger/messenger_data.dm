...
GLOBAL_LIST_EMPTY_TYPED(pda_messengers, /datum/computer_file/program/pdamessager)

/// Registers an NTMessenger instance to the list of pda_messengers.
/proc/add_messenger(datum/computer_file/program/pdamessager/person)
	var/obj/item/modular_computer/messenger_device = person.computer
	// a bunch of empty PDAs are normally allocated, we don't want that clutter
	if(!messenger_device.update_label())
		return

	if(!istype(person))
		return

	var/messenger_ref = REF()
	if(messenger_ref in GLOB.pda_messengers)
		return

	GLOB.pda_messengers[messenger_ref] = person

/// Unregisters an NTMessenger instance from the pda_messengers table.
/proc/remove_messenger(datum/computer_file/program/pdamessager/gone)
	if(!istype(gone))
		return

	var/messenger_ref = REF(gone)
	if(!(messenger_ref in GLOB.pda_messengers))
		return
...
/proc/get_messenger_name(datum/computer_file/program/pdamessager/messenger)
	if(!istype(messenger))
		return null
...
/datum/pda_chat/New(datum/computer_file/program/pdamessager/recipient)
...
/datum/pda_chat/proc/get_recipient_name()
	var/datum/computer_file/program/pdamessager/messenger = recipient?.resolve()
	if(istype(messenger) && (recipient.reference in GLOB.pda_messengers))
		cached_name = messenger.computer.update_label()
	return cached_name
...
/datum/pda_chat/proc/get_recipient_job()
	var/datum/computer_file/program/pdamessager/messenger = recipient?.resolve()
	if(istype(messenger) && (recipient.reference in GLOB.pda_messengers))
		cached_job = messenger.computer.update_label()
	return cached_job
