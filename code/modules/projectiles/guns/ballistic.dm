...
/obj/item/gun/ballistic/proc/rack(mob/user = null)
	...
	if (bolt_type == BOLT_TYPE_OPEN)
		if(!bolt_locked)	//If it's an open bolt, racking again would do nothing
			...
			return
		bolt_locked = FALSE
	if (user && do_after(user, 0, src, stayStill = FALSE, required_skill = required_skill, required_skill_level = required_skill_level, skill_delay_mult_scaling = skill_reload_delay))
...
/obj/item/gun/ballistic/proc/insert_magazine(mob/user, obj/item/ammo_box/magazine/AM, display_message = TRUE)
	if(!istype(AM, mag_type))
		...
		return FALSE
	if(user.transferItemToLoc(AM, src) && do_after(user, 0, src, stayStill = FALSE, required_skill = required_skill, required_skill_level = required_skill_level, skill_delay_mult_scaling = skill_reload_delay))
...
/obj/item/gun/ballistic/attackby(obj/item/A, mob/user, params)
	...
	if (istype(A, /obj/item/ammo_casing) || istype(A, /obj/item/ammo_box))
		if (bolt_type == BOLT_TYPE_NO_BOLT || internal_magazine)
			if(do_after(user, 0, src, stayStill = FALSE, required_skill = required_skill, required_skill_level = required_skill_level, skill_delay_mult_scaling = skill_reload_delay))
