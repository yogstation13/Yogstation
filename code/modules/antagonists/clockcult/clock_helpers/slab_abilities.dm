...
/obj/effect/proc_holder/slab/compromise/InterceptClickOn(mob/living/caller, params, atom/target)
	...
	if(isliving(target) && (target in view(7, get_turf(ranged_ability_user))))
		...
		var/healseverity = max(round(totaldamage*0.05, 1), 1) //shows the general severity of the damage you just healed, TRUE glow per 20
		for(var/i in TRUE to healseverity)
			new /obj/effect/temp_visual/heal(targetturf, "#1E8CE1")
		if(totaldamage)
			L.adjustBruteLoss(-brutedamage, TRUE, TRUE)
			L.adjustFireLoss(-burndamage, TRUE, TRUE)
