...
/obj/structure/clockwork/trap/trigger/pressure_sensor
...
/obj/structure/clockwork/trap/trigger/Initialize()
	. = ..()
	for(var/obj/structure/clockwork/trap/T in get_turf(src))
		if(!istype(T, /obj/structure/clockwork/trap/trigger))
			wired_to += T
			T.wired_to += src
			to_chat(usr, "<span class='alloy'>[src] automatically links with [T] beneath it.</span>")

/obj/structure/clockwork/trap/trigger/pressure_sensor/Crossed(atom/movable/AM)
	. = ..()
