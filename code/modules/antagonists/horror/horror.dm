...
/mob/living/simple_animal/horror/proc/ConsumeSoul()
	...
	if(do_after(src, 30 SECONDS, victim, timed_action_flags = IGNORE_USER_LOC_CHANGE))
		consume()
...
/mob/living/simple_animal/horror/proc/release_victim()
	...
	leaving = TRUE
	if(do_after(src, 10 SECONDS, victim, timed_action_flags = IGNORE_USER_LOC_CHANGE, extra_checks = CALLBACK(src, PROC_REF(is_leaving))))
		release_host()
...
/mob/living/simple_animal/horror/proc/bond_brain()
	...
	if(has_upgrade("fast_control"))
		delay -= 12 SECONDS
	if(do_after(src, delay, victim, timed_action_flags = IGNORE_USER_LOC_CHANGE, extra_checks = CALLBACK(src, PROC_REF(is_bonding))))
		assume_control()
