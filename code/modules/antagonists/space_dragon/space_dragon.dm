...
/datum/antagonist/space_dragon/greet()
	. = ..()
	to_chat(owner, 
		"<b>Through endless time and space we have moved. \
		We do not remember from where we came, we do not know where we will go. All of space belongs to us.\n\
...
/datum/antagonist/space_dragon/proc/destroy_rifts()
	if(objective_complete)
		return
	rifts_charged = 0
	owner.current.add_movespeed_modifier(MOVESPEED_ID_SPACE_DRAGON_SAD, TRUE, 101, override = TRUE, multiplicative_slowdown = 5)
	riftTimer = -1
...
/datum/antagonist/space_dragon/proc/permanant_empower()
	...
	owner.current.add_movespeed_modifier(MOVESPEED_ID_SPACE_DRAGON_RAGE, TRUE, 101, override = TRUE, multiplicative_slowdown = -0.5)
...
/datum/antagonist/space_dragon/proc/rift_depower()
	...
	owner.current.remove_movespeed_modifier(MOVESPEED_ID_SPACE_DRAGON_RAGE)
