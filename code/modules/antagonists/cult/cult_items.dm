...
/obj/item/twohanded/cult_spear
	...
	force = 17
	force_wielded = 24
	throwforce = 35 //yogs reduced from 40 cause wew
	throw_speed = 2
...
/obj/item/shield/mirror/hit_reaction(mob/living/carbon/human/owner, atom/movable/hitby, attack_text = "the attack", final_block_chance = 0, damage = 0, attack_type = MELEE_ATTACK)
	if(iscultist(owner))
		if(istype(hitby, /obj/item/projectile))
			...
			if(P.damage_type == BRUTE) //yogs removed burn check
	...
	if(isliving(hit_atom))
		...
		if(iscultist(L))
			...
			if(!L.anti_magic_check())
				if(is_servant_of_ratvar(L))
					L.Paralyze(60)
					break_mirror(T) //yogs added a break mirror proc
				else
					L.Paralyze(30)
					break_mirror(T) //yogs added a break mirror proc
				if(D?.thrower)
...
obj/item/shield/mirror/proc/break_mirror(turf/T) //yogs mirror proc start
	if(src)
		if(!T)
			T = get_turf(src)
