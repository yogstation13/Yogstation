...
/datum/antagonist/traitor/internal_affairs/proc/greet_iaa()
	...
	if(syndicate)
		...
		to_chat(owner.current, "<B><font size=5 color=red>Any damage you cause will further increase tenions between The Syndicate and Nanotrasen, so you have should limit collateral damage.</font></B>") //yogs - murderbone rule exists, apparently
