...
/datum/blobstrain/reagent/synchronous_mesh
	...
	analyserdescdamage = "Does brute damage, increasing for each blob near the target."
	analyserdesceffect = "When attacked, spreads damage between all blobs near the attacked blob."
	color = "#65ADA2"
...
/datum/reagent/blob/synchronous_mesh/reaction_mob(mob/living/M, methods = TOUCH, reac_volume, show_message, permeability, mob/camera/blob/O)
	reac_volume = ..()
