...
/datum/reagent/blob/cryogenic_poison/reaction_mob(mob/living/M, methods = TOUCH, reac_volume, show_message, permeability, mob/camera/blob/O)
	reac_volume = ..()
	if(M.reagents)
		...
		M.reagents.add_reagent(/datum/reagent/blob/cryogenic_poison, 0.6*reac_volume)
