...
/datum/round_event_control/shuttle_catastrophe/canSpawnEvent(players, gamemode)
	if((SSshuttle.emergency.name == "Build Your Own Shuttle") || (SSshuttle.emergency.mode == SHUTTLE_CALL || SSshuttle.emergency.mode == SHUTTLE_RECALL || SSshuttle.emergency.mode == SHUTTLE_DOCKED || SSshuttle.emergency.mode == SHUTTLE_STRANDED))
