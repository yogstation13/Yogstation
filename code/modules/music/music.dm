...
/datum/music/New(client/C, _fade_volume = 1)
	...
	if((target.prefs.toggles & SOUND_JUKEBOX) && (!target.playing_music || target.playing_music.priority < priority || (target.playing_music.priority == priority && target.playing_music.does_loop && !does_loop)))
		unmask()
	else if(!does_loop)
		qdel(src)
...
/client/proc/update_playing_music()
	if(!(prefs.toggles & SOUND_JUKEBOX))
		if(playing_music)
			playing_music.mask()
...
/datum/music/sourced/battle/king_goat
	...
	priority = 200
/datum/music/sourced/battle/king_goat_2
	sound_file = 'sound/lavaland/music/kilogram2.ogg'
	priority = 210
