...
/datum/surgery_step/proc/initiate(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, try_to_fail = FALSE)
	...
	if(do_after(user, time * speed_mod, target = target))
		...
		if((prob(prob_chance) || iscyborg(user)) && chem_check(target, tool) && !try_to_fail)
...
/datum/surgery_step/proc/chem_check(mob/living/carbon/target, obj/item/tool)
	if(!LAZYLEN(chems_needed))
		return TRUE
	var/obj/item/reagent_container/syringe/S
