			...
			if(prob(1))
				if(owner.stat == CONSCIOUS)
					if(owner.get_num_arms(FALSE) >= 1) //gotta have an arm to clutch your chest
						...
						owner.visible_message(span_userdanger("[owner] clenches [owner.p_their()] jaw[owner.get_organ_slot(ORGAN_SLOT_EYES) ? " and stares off into space." : "."]")) //ok you also need eyes
	...
	if(organ_flags & ORGAN_FAILING)	//heart broke, stopped beating, death imminent
		if(owner.stat == CONSCIOUS)
			if(owner.get_num_arms(FALSE) >= 1)
				...
				owner.visible_message(span_userdanger("[owner] clenches [owner.p_their()] jaw[owner.get_organ_slot(ORGAN_SLOT_EYES) ? " and stares off into space." : "."]"))
