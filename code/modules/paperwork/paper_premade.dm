...
/obj/item/paper/fluff/stations/centcom/bulletin
	...
	info = "<BR>CentCom Security<BR>Port Division<BR>Official Bulletin<BR><BR>Inspector,<BR>There is an emergency shuttle arriving today.<BR><BR>Approval is restricted to yogscast employees only. Deny all other entrants.<BR><BR>CentCom Port Commissioner"
	infolang = /datum/language/common
