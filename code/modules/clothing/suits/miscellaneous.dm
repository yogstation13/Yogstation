...
/obj/item/clothing/suit/priest_robe/ui_action_click(mob/user, action)
	if(!ishuman(user))
	...
	if(!religious_color)
		...
		if(new_color)
			var/s = "/obj/item/clothing/suit/priest_robe/[lowertext(new_color)]"
			if(istype(H.get_item_by_slot(SLOT_WEAR_SUIT), /obj/item/clothing/suit/priest_robe))
	...
	if(istype(action,/datum/action/item_action/priest_wololo))
		...
		for(var/mob/living/carbon/human/L in range(conversion_range, user))
			if(istype(L.wear_suit, /obj/item/clothing/suit/priest_robe))
...
/obj/item/clothing/suit/priest_robe/red
...
/obj/item/clothing/suit/priest_robe/blue
...
/obj/item/clothing/suit/leather_coat
	name = "scavenged leather coat"
	desc = "Clearly not made by an expert, but what do you expect in space and with no leather working tools?"
	icon_state = "leather_coat"
	item_state = "leather_coat"

/obj/item/clothing/suit/cleric_robe
	name = "cleric robes"
