...
/datum/controller/subsystem/ticker/proc/survivor_report(popcount)
	...
	if(total_players)
		...
		if(SSblackbox.first_)
			var/list/ded = SSblackbox.first_
			if(ded.len)
				parts += "[GLOB.TAB]First : <b>[ded["name"]], [ded["role"]], at [ded["area"]]. Damage taken: [ded["damage"]].[ded["last_words"] ? " Their last words were: \"[ded["last_words"]]\"" : ""]</b>"
