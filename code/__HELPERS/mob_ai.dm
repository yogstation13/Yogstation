...
/proc/get_best_weapon(datum/ai_controller/controller, list/choices, list/held_weapons)
	...
	for(var/obj/item/item as anything in held_weapons)
		if(!item)
			continue
		if(istype(item, /obj/item/twohanded/required) || controller.blackboard[BB_MONKEY_BLACKLISTITEMS][item])
			continue
...
//		if(HAS_TRAIT(item, TRAIT_NEEDS_TWO_HANDS) || controller.blackboard[BB_MONKEY_BLACKLISTITEMS][item])
		if(istype(item, /obj/item/twohanded/required) || controller.blackboard[BB_MONKEY_BLACKLISTITEMS][item]) //this actually sucks balls
			continue
		if(gun_neurons_activated && isgun(item))
			return item
		if(item.force <= top_force)
