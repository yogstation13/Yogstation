...
/datum/atmosphere/proc/generate_gas_string()
	...
	while(gasmix.return_pressure() > target_pressure)
		var/cur_moles = gasmix.get_moles(gastype)
		gasmix.set_moles(gastype, cur_moles - cur_moles*0.1)
	gasmix.set_moles(gastype, FLOOR(gasmix.get_moles(gastype), 0.1))

	// Now finally lets make that string
	var/list/gas_string_builder = list()
	for(var/gas in gasmix.get_gases())
		gas_string_builder += "[GLOB.meta_gas_info[gas][META_GAS_ID]]=[gasmix.get_moles(gas)]"
