...
/datum/component/summoning/Initialize(mapload, mob_types, spawn_chance=100, max_mobs=3, spawn_delay=100, spawn_text="appears out of nowhere", spawn_sound='sound/magic/summon_magic.ogg', faction)
...
/datum/component/summoning/proc/do_spawn_mob(atom/spawn_location, summoner)
	...
	if(faction != null)
		L.faction = faction
	RegisterSignal(L, COMSIG_GLOB_MOB_DEATH, PROC_REF(on_spawned_death)) // so we can remove them from the list, etc (for mobs with corpses)
