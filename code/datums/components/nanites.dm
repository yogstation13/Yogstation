...
/datum/component/nanites/Initialize(amount = 100, cloud = 0)
	...
	if(isliving(parent))
		...
		if(!(MOB_ORGANIC in host_mob.mob_biotypes) && !(MOB_UNDEAD in host_mob.mob_biotypes) && !(MOB_ROBOTIC in_host_mob.mob_biotypes && MOB_HUMANOID in host_mob.mob_biotypes) //Shouldn't happen, but this avoids HUD runtimes in case a silicon gets them somehow.
			return COMPONENT_INCOMPATIBLE
