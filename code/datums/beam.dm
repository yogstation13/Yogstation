...
/datum/beam/proc/Draw()
	...
	for(N in FALSE to length-1 step 32)//-1 as we want < not <=, but we want the speed of X in Y to Z and step X
		...
		var/a
		if(abs(Pixel_x)>32)
			a = Pixel_x > FALSE ? round(Pixel_x/32) : CEILING(Pixel_x/32, 1)
			X.x += a
			Pixel_x %= 32
		if(abs(Pixel_y)>32)
			a = Pixel_y > FALSE ? round(Pixel_y/32) : CEILING(Pixel_y/32, 1)
			X.y += a
			Pixel_y %= 32
