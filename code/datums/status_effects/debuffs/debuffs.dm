...
/datum/status_effect/demonic_hallucination
	...
	var/datum/brain_trauma/severe/split_personality/split

/atom/movable/screen/alert/status_effect/demonic_hallucination
	name = "Demonic hallucination"
	desc = "You find yourself engulfed in a disquieting hallucination of a demonic nature, where your mind becomes unsteady and an intense foreboding arises, signaling the imminent arrival of an otherworldly entity."
	icon = 'icons/effects/effects.dmi'
	icon_state = "curseblob"

/datum/status_effect/demonic_hallucination/on_apply()
	if(iscarbon(owner))
		var/mob/living/carbon/carbon = owner
		if(!carbon.has_trauma_type(/datum/brain_trauma/magic/stalker))
			stalk = new()
			carbon.gain_trauma(stalk, TRAUMA_RESILIENCE_ABSOLUTE)
		if(!carbon.has_trauma_type(/datum/brain_trauma/magic/poltergeist))
			pol = new()
			carbon.gain_trauma(pol, TRAUMA_RESILIENCE_ABSOLUTE)
		if(!carbon.has_trauma_type(/datum/brain_trauma/severe/split_personality))
			split = new()
			carbon.gain_trauma(split, TRAUMA_RESILIENCE_ABSOLUTE)
		
	return ..()

/datum/status_effect/demonic_hallucination/tick()
	if(prob(2))
		var/turf/T = get_turf(owner)
		T.handle_slip(owner, 100)
		owner.Stun(50)
		to_chat(owner, span_danger("You feel like something grabs your foot, making you tripped over!"))
	if(prob(10))
		var/spooky_sound = pick(GLOB.ambience_assoc[AMBIENCE_CREEPY])
		SEND_SOUND(owner, spooky_sound)

/datum/status_effect/demonic_hallucination/on_remove()
	if(split)
		QDEL_NULL(split)
	if(pol)
		QDEL_NULL(pol)
