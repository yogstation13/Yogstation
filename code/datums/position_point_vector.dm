...
/datum/point		//A precise point on the map in absolute pixel locations based on WORLD_ICON_SIZE. Pixels are FROM THE EDGE OF THE MAP!
	var/x = 0
	var/y = 0
	var/z = 0
...
/datum/point/proc/return_turf()
	return locate(CEILING(x / WORLD_ICON_SIZE, 1), CEILING(y / WORLD_ICON_SIZE, 1), z)

/datum/point/proc/return_coordinates()		//[turf_x, turf_y, z]
	return list(CEILING(x / WORLD_ICON_SIZE, 1), CEILING(y / WORLD_ICON_SIZE, 1), z)

/datum/point/proc/return_position()
	return new /datum/position(src)

/datum/point/proc/return_px()
	return MODULUS(x, WORLD_ICON_SIZE) - 16 - 1

/datum/point/proc/return_py()
	return MODULUS(y, WORLD_ICON_SIZE) - 16 - 1
