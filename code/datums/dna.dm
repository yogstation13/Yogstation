...
/datum/dna/proc/update_ui_block(blocknumber)
	...
	switch(blocknumber)
		if(DNA_HAIR_COLOR_BLOCK)
			unique_identity = set_uni_identity_block( blocknumber, sanitize_hexcolor(H.hair_color, include_crunch = FALSE))
		if(DNA_FACIAL_HAIR_COLOR_BLOCK)
			unique_identity = set_uni_identity_block( blocknumber, sanitize_hexcolor(H.facial_hair_color, include_crunch = FALSE))
		if(DNA_SKIN_TONE_BLOCK)
			unique_identity = set_uni_identity_block( blocknumber, construct_block(GLOB.skin_tones.Find(H.skin_tone), GLOB.skin_tones.len))
		if(DNA_EYE_COLOR_BLOCK)
			unique_identity = set_uni_identity_block( blocknumber, sanitize_hexcolor(H.eye_color, include_crunch = FALSE))
		if(DNA_GENDER_BLOCK)
			switch(H.gender)
				if(MALE)
					unique_identity = set_uni_identity_block( blocknumber, construct_block(G_MALE, 3))
				if(FEMALE)
					unique_identity = set_uni_identity_block( blocknumber, construct_block(G_FEMALE, 3))
				else
					unique_identity = set_uni_identity_block( blocknumber, construct_block(G_PLURAL, 3))
		if(DNA_FACIAL_HAIR_STYLE_BLOCK)
			unique_identity = set_uni_identity_block( blocknumber, construct_block(GLOB.facial_hair_styles_list.Find(H.facial_hair_style), GLOB.facial_hair_styles_list.len))
		if(DNA_HAIR_STYLE_BLOCK)
			unique_identity = set_uni_identity_block( blocknumber, construct_block(GLOB.hair_styles_list.Find(H.hair_style), GLOB.hair_styles_list.len))
...
/mob/living/carbon/human/updateappearance(icon_update=1, mutcolor_update=0, mutations_overlay_update=0)
	...
	if(dna.features["pod_hair"])
		dna.features["pod_hair"] = GLOB.pod_hair_list[deconstruct_block(get_uni_feature_block(features, DNA_PLANT_HAIR_BLOCK), GLOB.pod_hair_list.len)]
	if(dna.features["pod_flower"])
		dna.features["pod_flower"] = GLOB.pod_flower_list[deconstruct_block(get_uni_feature_block(features, DNA_PLANT_FLOWER_BLOCK), GLOB.pod_flower_list.len)]
