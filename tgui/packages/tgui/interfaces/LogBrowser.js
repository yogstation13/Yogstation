...
const generate_server_list = (servers,act) => {
    if(!servers || servers.length === 0)
    {
        return (
            ...
            <Icon name='exclamation-triangle' size={5}></Icon><br/>
...
const generate_logs = (logs,act) => {
    ...
    for (const [index, log] of Object.entries(logs)) {
        if(log["is_error"])
        {
            ...
            <Icon name='exclamation-triangle' size={2}></Icon><br/>
            {log["message"]}
            </NoticeBox>);
        }
        else
...
export const LogBrowser = (props, context) => {
    ...
    return (
        <Window 
            ...
            <Window.Content>
            <Box>
                Current network: <Input placeholder={network ? network : "NO NETWORK"} onChange={ (e,value) => {act('SetNetwork',{"network": value})}}></Input>
            </Box>
            <Divider></Divider>
            <Button onClick={(e,value) => {act('Scan',{})}}><Icon name='binoculars'></Icon>Scan for Servers</Button>
            <Section>
    ...
    return (
        <Window 
            ...
            <Button onClick={(e,value) => {act('Back',{})}}><Icon name='backward'></Icon>Back</Button>
            <Divider></Divider>
            <Section title="Server Information">
            <Icon name='wifi' size={1}></Icon>&emsp;Network:<br/> {network}<br/><br/>
            <Icon name='server' size={1}></Icon>&emsp;Server:<br/>{selected_name}<br/><br/>
            <Box position="relative">
            <Icon name='project-diagram'></Icon>&emsp;Total Traffic Storage:<Tooltip
            ...
            <ProgressBar
                ...
                value={totaltraffic}>{totaltraffic} GB</ProgressBar>
            </Box>
            </Section>
