...
namespace CreditsTool
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            {
                Console
                    .WriteLine("Usage: ./CreditsTool.exe <repo owner> <repo> [authToken]");
                return;
            }
            ...
            if (Directory.Exists(OutputLocation))
            {
                Console
                    .WriteLine(String
                        .Format("Aborted: {0} exists!", OutputLocation));
                return;
            }
            ...
            var FirstResponse =
                GetPageResponse(repoOwner, repoName, authToken, 1);
            ...
            foreach (var
                I
                in
                LoadPages(FirstResponse, repoOwner, repoName, authToken)
            )
            {
            ...
            Console
                .WriteLine(String
                    .Format("Collected info for {0} contributors.",
                    LoginAvatars.Count));
            ...
            Console
                .WriteLine(String
                    .Format("Downloading and converting avatars to {0} (this will take a while)...",
                    OutputLocation));
            ...
            using (var client = new WebClient())
            {
                ...
                foreach (var I in LoginAvatars)
                {
                    ...
                    {
                        if (tmp == "__REMOVE__") continue;
                        writtenFilename = tmp;
                    }
                    using (
                        var stream =
                            new MemoryStream(client.DownloadData(I.Value))
                    )
                    using (var originalBMP = new Bitmap(stream))
                    {
                        if (
                            originalBMP.Width == world_icon_size &&
                            originalBMP.Height == world_icon_size //no need to resize
                        )
                            SaveBMP(originalBMP, writtenFilename);
                        else
                            using (
                                var resizedBMP =
                                    new Bitmap(originalBMP,
                                        new Size(world_icon_size,
                                            world_icon_size))
                            )
                    ...
                    Console
                        .WriteLine(String
                            ...
                            (
                            int
                            )((((float)(count + 1)) / LoginAvatars.Count) *
                            100)));
                    ++count;
                }
            }
        ...
        static IEnumerable<IDictionary<string, object>>
        LoadPages(
            WebResponse firstResponse,
            string repoOwner,
            string repoName,
            string authToken
        )
        {
            ...
            Console
                .WriteLine(String
                    .Format("Downloading {0} pages of contributor info...",
                    numPages));
            ...
            using (var sr = new StreamReader(firstResponse.GetResponseStream()))
                foreach (var
                    ...
                    jss
                        .Deserialize
                        <IEnumerable<IDictionary<string, object>>>(sr
                            .ReadToEnd())
                )
                yield return J;
            ...
            using (
                var sr =
                    new StreamReader(GetPageResponse(repoOwner,
                        repoName,
                        authToken,
                        I).GetResponseStream())
            )
        ...
        )
        {
            HttpWebRequest httpWebRequest =
                ...
                WebRequest
                    .Create(String
                        .Format("https://api.github.com/repos/{0}/{1}/contributors?per_page=100&page={2}",
                        repoOwner,
                        repoName,
                        pageNumber));
            ...
            if (authToken != null)
                httpWebRequest
                    .Headers
                    .Add(String.Format("Authorization: token {0}", authToken));
