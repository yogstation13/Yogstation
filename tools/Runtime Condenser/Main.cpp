	...
	for (size_t i = pos; i < LINEBUFFER; i++)
	{
		...
		if (i >= size && (pos || i < LINEBUFFER - 1))
		{
			if (feof(f) || ferror(f))
				break;
			if (size && pos)
			{ //move current stuff to start of buffer
				size -= pos;
				i -= pos;
	...
	if (usestdio || outputFile->is_open())
	{
		*output << "Note: The source file, src and usr are all from the FIRST of the identical runtimes. Everything else is cropped.\n\n";
		...
		if (totalInfiniteLoops > 0)
			*output << "Total infinite loops: " << totalInfiniteLoops << endl
					<< endl;

		*output << "Total unique runtimes: " << storedRuntime.size() << endl;
		*output << "Total runtimes: " << totalRuntimes << endl
				<< endl;
		if (storedHardDel.size() > 0)
			*output << "Total unique hard deletions: " << storedHardDel.size() << endl;

		if (totalHardDels > 0)
			*output << "Total hard deletions: " << totalHardDels << endl
					<< endl;
		...
		if (storedInfiniteLoop.size() > 0)
		{
			...
			*output << "** Infinite loops **";
			for (int i = 0; i < infiniteLoops.size(); i++)
			{
				...
				*output << endl
						<< endl
						<< "The following infinite loop has occurred " << R->count << " time(s).\n";
				*output << R->text << endl;
				if (R->proc.length())
					*output << R->proc << endl;
				if (R->source.length())
		...
		*output << "** Runtimes **";
		...
		for (int i = 0; i < runtimes.size(); i++)
		{
			...
			*output << endl
					<< endl
					<< "The following runtime has occurred " << R->count << " time(s).\n";
			*output << R->text << endl;
			if (R->proc.length())
				*output << R->proc << endl;
			if (R->source.length())
