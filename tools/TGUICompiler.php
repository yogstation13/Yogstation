...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($the_count > 0 && $the_count < $max_number_of_uploads) {
            ...
            exec(
                'mklink /j "' .
                    str_replace('/', '\\', $target_node) .
                    '" "' .
                    str_replace('/', '\\', $parent_node) .
                    '"'
            );
...
<html>
	...
	<body>
		<?php if (isset($error)) {
      echo '<h5><font color="red">An error occured:</font> ' .
          $error .
          '</h5><br><br>';
  } ?>
		<h1>Upload up to <?php echo $max_number_of_uploads; ?> .ract files<h2>
		<h4>Based off revision: <?php echo isset($commit_url)
      ? '<a href="' . $commit_url . '">' . $revision . '</a>'
      : $revision; ?>
