...
switch (strtolower($_SERVER['HTTP_X_GITHUB_EVENT'])) {
    ...
    case 'pull_request_review':
        if ($payload['action'] == 'submitted') {
            ...
            if (
                ($lower_state == 'approved' ||
                    $lower_state == 'changes_requested') &&
                is_maintainer($payload, $payload['review']['user']['login'])
            ) {
...
function tag_pr($payload, $opened)
{
    ...
    $title = $payload['pull_request']['title'];
    if ($opened) {
        //you only have one shot on these ones so as to not annoy maintainers
    ...
    $remove = ['Test Merge Candidate'];

    $mergeable = $payload['pull_request']['mergeable'];
    if ($mergeable === true) {
        //only look for the false value
        $remove[] = 'Merge Conflict';
    } elseif ($mergeable === false) {
        $tags[] = 'Merge Conflict';
    }
...
function dismiss_review($payload, $id, $reason)
{
    ...
    github_apisend(
        $payload['pull_request']['url'] . '/reviews/' . $id . '/dismissals',
        'PUT',
        $content
    );
}
...
{
    return json_decode(
        github_apisend($payload['pull_request']['url'] . '/reviews'),
        true
    );
}

function check_ready_for_review($payload, $labels = null, $remove = [])
{
    $r4rlabel = 'Needs Review';
    $labels_which_should_not_be_ready = [
        'Do Not Merge',
        'Work In Progress',
        'Merge Conflict',
    ];
    ...
    if (
        !$dismissed_an_approved_review &&
        count($reviews_ids_with_changes_requested) == 0
    ) {
    ...
    if (count($reviews_ids_with_changes_requested) > 0) {
        ...
        $review_comments = json_decode(
            github_apisend($payload['pull_request']['review_comments_url']),
            true
        );
...
function check_dismiss_changelog_review($payload)
{
    ...
    $review_message =
        'Your changelog for this PR is either malformed or non-existent. Please create one to document your changes.';
    ...
    if ($no_changelog) {
        ...
        foreach ($reviews as $R) {
            if (
                $R['body'] == $review_message &&
                strtolower($R['state']) == 'changes_requested'
            ) {
                return;
            }
        }
        //otherwise make it ourself
        github_apisend($payload['pull_request']['url'] . '/reviews', 'POST', [
            'body' => $review_message,
...
function handle_pr($payload)
{
    ...
    switch ($payload['action']) {
        case 'opened':
            ...
            if (get_pr_code_friendliness($payload) <= 0) {
                ...
                if (
                    isset($balances[$author]) &&
                    $balances[$author] < 0 &&
                    !is_maintainer($payload, $author)
                ) {
                    create_comment(
                        $payload,
                        'You currently have a negative Fix/Feature pull request delta of ' .
                            $balances[$author] .
                            '. Maintainers may close this PR at will. Fixing issues or improving the codebase will improve this score.'
                    );
                }
            }
            break;
        ...
        case 'synchronize':
            ...
            if ($payload['action'] == 'synchronize') {
                list($labels, $remove) = check_ready_for_review(
                    $payload,
                    $labels,
                    $remove
                );
    ...
    $pr_flags = 0;
    if (
        strpos(strtolower($payload['pull_request']['title']), '[s]') !== false
    ) {
...
function filter_announce_targets($targets, $owner, $repo, $action, $pr_flags)
{
    foreach ($targets as $i => $target) {
        if (
            isset($target['exclude_events']) &&
            in_array(
                $action,
                array_map('strtolower', $target['exclude_events'])
            )
        ) {
            unset($targets[$i]);
            continue;
        ...
        if (isset($target['announce_secret']) && $target['announce_secret']) {
            if (
                !($pr_flags & F_SECRET_PR) &&
                $target['announce_secret'] === 'only'
            ) {
                unset($targets[$i]);
                continue;
            }
        } elseif ($pr_flags & F_SECRET_PR) {
            unset($targets[$i]);
            continue;
        }

        if (
            isset($target['announce_unvalidated']) &&
            $target['announce_unvalidated']
        ) {
            if (
                !($pr_flags & F_UNVALIDATED_USER) &&
                $target['announce_unvalidated'] === 'only'
            ) {
                unset($targets[$i]);
                continue;
            }
        } elseif ($pr_flags & F_UNVALIDATED_USER) {
            unset($targets[$i]);
            continue;
        }
            ...
            foreach ($target['include_repos'] as $match_string) {
                ...
                if (strtolower($owner) == $owner_repo_pair[0]) {
                    if (strtolower($repo) == $owner_repo_pair[1]) {
                        continue 2;
                    } //don't parse excludes when we have an exact include match
            ...
            foreach ($target['exclude_repos'] as $match_string) {
                ...
                if (strtolower($owner) == $owner_repo_pair[0]) {
                    ...
                    if ($owner_repo_pair[1] == '*') {
                        if ($wildcard) {
                            log_error(
                                'Identical wildcard include and exclude: `' .
                                    $match_string .
                                    '`. Excluding.'
                            );
                        }
...
function game_announce($action, $payload, $pr_flags)
{
    ...
    $msg =
        ...
        $action .
        ' by ' .
        htmlSpecialChars($payload['sender']['login']) .
        ': <a href="' .
    ...
    $game_servers = filter_announce_targets(
        $servers,
        $payload['pull_request']['base']['repo']['owner']['login'],
...
function discord_announce($action, $payload, $pr_flags)
{
    ...
    $content =
        ...
        $action .
        '* by ' .
        discord_sanitize($payload['sender']['login']) .
        "\n" .
        discord_sanitize($payload['pull_request']['user']['login']) .
        ' - __**' .
        discord_sanitize($payload['pull_request']['title']) .
        '**__' .
        "\n" .
        '<' .
        $payload['pull_request']['html_url'] .
    ...
    $embeds = [
        [
            'title' =>
                ...
                '**__',
            'description' => discord_sanitize(
                str_replace(
                    ["\r\n", "\n"],
                    [' ', ' '],
                    substr($payload['pull_request']['body'], 0, 320)
                ),
                S_HTML_COMMENTS
            ),
            ...
            'footer' => [
                ...
                'icon_url' =>
                    $payload['pull_request']['base']['user']['avatar_url'],
            ],
        ],
    ];
    $discordWebHook_targets = filter_announce_targets(
        $discordWebHooks,
        $payload['pull_request']['base']['repo']['owner']['login'],
        $payload['pull_request']['base']['repo']['name'],
        $action,
    ...
    foreach ($discordWebHook_targets as $discordWebHook) {
        ...
        if (isset($discordWebHook['embed']) && $discordWebHook['embed']) {
            ...
            if (
                !isset($discordWebHook['no_text']) ||
                !$discordWebHook['no_text']
            ) {
...
function discord_sanitize(
    $text,
    $flags = S_MENTIONS | S_LINK_EMBED | S_MARKDOWN
) {
    if ($flags & S_MARKDOWN) {
        $text = str_ireplace(
            ['\\', '*', '_', '~', '`', '|'],
    ...
    if ($flags & S_LINK_EMBED) {
        $text = preg_replace(
            "/((https?|ftp|byond)\:\/\/)([a-z0-9-.]*)\.([a-z]{2,3})(\:[0-9]{2,5})?(\/(?:[a-z0-9+\$_-]\.?)+)*\/?(\?[a-z+&\$_.-][a-z0-9;:@&%=+\/\$_.-]*)?(#[a-z_.-][a-z0-9+\$_.-]*)?/mi",
            '<$0>',
            $text
        );
    }
...
function get_pr_code_friendliness($payload, $oldbalance = null)
{
    ...
    foreach ($labels as $l) {
        if ($l == 'PRB: No Update') {
            ...
            $affecting = 0;
            break;
        } elseif (isset($label_values[$l])) {
            $friendliness = $label_values[$l];
            if ($friendliness > 0) {
                $found_something_positive = true;
            }
            $affecting = $found_something_positive
                ? max($affecting, $friendliness)
                : $friendliness;
        }
    }
    return $affecting;
...
function is_maintainer($payload, $author)
{
    ...
    $repo_is_org =
        $payload['pull_request']['base']['repo']['owner']['type'] ==
        'Organization';
    if ($maintainer_team_id == null || !$repo_is_org) {
        $collaburl =
            str_replace(
                '{/collaborator}',
                '/' . $author,
                $payload['pull_request']['base']['repo']['collaborators_url']
            ) . '/permission';
...
function update_pr_balance($payload)
{
    ...
    if (!is_maintainer($payload, $author)) {
        //immune
        if ($balances[$author] < 0 && $friendliness < 0) {
            create_comment(
                $payload,
                'Your Fix/Feature pull request delta is currently below zero (' .
                    $balances[$author] .
                    '). Maintainers may close future Feature/Tweak/Balance PRs. Fixing issues or helping to improve the codebase will raise this score.'
            );
        } elseif (
            $balances[$author] >= 0 &&
            $balances[$author] - $friendliness < 0
        ) {
            create_comment(
                $payload,
                'Your Fix/Feature pull request delta is now above zero (' .
                    $balances[$author] .
                    '). Feel free to make Feature/Tweak/Balance PRs.'
            );
        }
    }
...
function auto_update($payload)
{
    ...
    if (
        ...
        !has_tree_been_edited($payload, $path_to_script) ||
        $payload['pull_request']['base']['ref'] != $tracked_branch
    ) {
        return;
    }
...
function has_tree_been_edited($payload, $tree)
{
    ...
    return $github_diff !== false &&
        preg_match(
            '/^diff --git a\/' . preg_quote($tree, '/') . '/m',
            $github_diff
        ) !== 0;
}
...
function checkchangelog($payload, $compile = true)
{
    ...
    if (
        ...
        !isset($payload['pull_request']['body'])
    ) {
        return;
    }
    if (
        !isset($payload['pull_request']['user']) ||
        !isset($payload['pull_request']['user']['login'])
    ) {
        return;
    }
    ...
    if (
        preg_match(
            '/(?i)(fix|fixes|fixed|resolve|resolves|resolved)\s*#[0-9]+/',
            $body
        )
    ) {
        //github autoclose syntax
        $tags[] = 'Fix';
    }
    ...
    foreach ($body as $line) {
        ...
        switch ($cltype) {
            ...
            case 'rscadd':
                if (
                    $item != 'Added new things' &&
                    $item != 'Added more things'
                ) {
                    $tags[] = 'Feature';
                    $currentchangelogblock[] = [
                        'type' => 'rscadd',
            ...
            case 'rebalance':
                ...
                break;
            case 'tgs':
                $currentchangelogblock[] = ['type' => 'tgs', 'body' => $item];
                break;
            case 'code_imp':
            case 'code':
            ...
            default:
                ...
                if (count($currentchangelogblock) > 0) {
                    $currentchangelogblock[count($currentchangelogblock) - 1][
                        'body'
                    ] .= "\n" . $line;
                }
                break;
    ...
    foreach ($changelogbody as $changelogitem) {
        ...
        $body = trim(
            str_replace(['\\', '"'], ['\\\\', "\\\""], $changelogitem['body'])
        );
        $file .= '  - ' . $type . ': "' . $body . '"';
        $file .= "\n";
    }
    $content = [
        ...
        'message' =>
            'Automatic changelog generation for PR #' .
            $payload['pull_request']['number'] .
            ' [ci skip]',
    ...
    $filename =
        '/html/changelogs/AutoChangeLog-pr-' .
        $payload['pull_request']['number'] .
        '.yml';
    echo github_apisend(
...
function game_server_send($addr, $port, $str)
{
    ...
    socket_set_option($server, SOL_SOCKET, SO_SNDTIMEO, [
        'sec' => 2,
        'usec' => 0,
    ]); //sets connect and send timeout to 2 seconds
    ...
    if ($result != '') {
        if ($result[0] == "\x00" || $result[1] == "\x83") {
            // make sure it's the right packet format
            ...
            if ($result[4] == "\x2a") {
                // 4-byte big-endian floating-point
                $unpackint = unpack(
                    'f',
                    $result[5] . $result[6] . $result[7] . $result[8]
                ); // 4 possible bytes: add them up together, unpack them as a floating-point
                return $unpackint[1];
            } elseif ($result[4] == "\x06") {
                // ASCII string
                ...
                while ($size > 0) {
                    // loop through the entire ASCII string
                    $size--;
